    ...
    public void addRendererMaker(AbstractRendererMaker maker)
    {
        ...
        for (int i = 0; i < mimes.length; i++)
        {
            ...
            if (rendererMakerMapping.get(mimes[i]) != null)
                LogUtil.debug("Overriding renderer for " + mimes[i] + " with " + maker.getClass().getName());
    ...
    public void addFOEventHandlerMaker(AbstractFOEventHandlerMaker maker)
    {
        ...
        for (int i = 0; i < mimes.length; i++)
        {
            ...
            if (eventHandlerMakerMapping.get(mimes[i]) != null)
            	LogUtil.debug("Overriding FOEventHandler for " + mimes[i] + " with " + maker.getClass().getName());
...
//            renderer.setRenderable(inputHandler); //set before user agent!
    ...
    /** 从 classes\META-INF\services\com.wisii.wisedoc.render.Renderer 文件中读取可用的各Renderer类并动态记录它们。     */
//del by huangzl
	/*
...
//	private void discoverRenderers()
	{
//del by huangzl.
//		addRendererMaker(new com.wisii.wisedoc.render.txt.TXTRendererMaker());
//		addRendererMaker(new com.wisii.wisedoc.render.awt.AWTRendererMaker());
//		addRendererMaker(new com.wisii.wisedoc.render.xml.XMLRendererMaker());
//		addRendererMaker(new com.wisii.wisedoc.render.print.PrintRendererMaker());
//del end.
	}
//add end
    ...
    if(providers != null)
    {
        while(providers.hasNext())
        {
            ...
            try
            {
                if(LogUtil.canDebug())
                {
                	LogUtil.debug("Dynamically adding maker for Renderer: "
            ...
            {
            	LogUtil.errorException("Error while adding maker for Renderer", e);
            }
        }
    }
}

    /** 从 classes\META-INF\services\com.wisii.wisedoc.fo.FOEventHandler 文件中读取可用的各FOEventHandler类并动态记录它们。    */
    private void discoverFOEventHandlers()
    {
        ...
        if (providers != null)
        {
            while (providers.hasNext())
            {
                ...
                {
                    if (LogUtil.canDebug())
                    	LogUtil.debug("Dynamically adding maker for FOEventHandler: " + maker.getClass().getName());
                    addFOEventHandlerMaker(maker);
                }
                catch (IllegalArgumentException e)
                {
                	LogUtil.errorException("Error while adding maker for FOEventHandler", e);
                }
            }
        }
    }
