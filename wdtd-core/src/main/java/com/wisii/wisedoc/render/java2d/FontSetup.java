 ...
 * com.wisii.wisedoc.render.fonts.FontSetup.
...
//del by lizhenyou 现在使用已经读取好的wddeconfig.properties
//        try 
//        {
//            URL url = FontSetup.class.getClassLoader().getResource("WDWEconfig.xml");
//            String path = null;
//            if(url == null)
//            {
//                System.err.println("读取WDWEconfig.xml文件出错！");
//            }
//            else 
//            {
...
////                _document = reader.read(new FileInputStream(path)); //初始化document对象变量.
//            }
//        }
//        catch(Exception ex)
//        {
...
//      del end
        /*
           HARDCODED_FONT_NAMES.add("Arial");
           HARDCODED_FONT_NAMES.add("Courier New");
           HARDCODED_FONT_NAMES.add("Times New Roman");
           HARDCODED_FONT_NAMES.add("仿宋_GB2312");
           HARDCODED_FONT_NAMES.add("黑体");
           HARDCODED_FONT_NAMES.add("楷体_GB2312");
           HARDCODED_FONT_NAMES.add("宋体");
        */
    }
...
*/
    ...
    private static void configureInstalledAWTFonts(FontInfo fontInfo, Graphics2D graphics,
        ...
        for (int i = 0; i < allFontFamilies.length; i++) {            String family = allFontFamilies[i];
            ...
            if (LogUtil.canDebug()) {
                LogUtil.debug("Registering: " + family);
            }
...
//        }
        ...
        String[] allfont = String.valueOf(fontMap.get("font.define")).split(",");

        for(int i=0;i<allfont.length;i++)
        {
            fontName=  String.valueOf(fontMap.get("font."+allfont[i]+".name"));
