...
public class DataTransformBand implements WiseBand
{
	...
	public JRibbonBand getBand()
	{
		...
		JCommandButton currentinline = new JCommandButton(
				RibbonUIText.DATA_TRANSFORM_TABLE, MediaResource
		...
		RibbonUIManager.getInstance().bind(
				Datatreat.SET_DATATREAT_TRANSFORM_TABLE, currentinline);
		visibleGroupBand.addCommandButton(currentinline,
				RibbonElementPriority.TOP);

		JCommandButton remove = new JCommandButton(
				RibbonUIText.REMOVE_DATA_TRANSFORM_TABLE, MediaResource
						.getResizableIcon("00906.ico"));
		remove.setCommandButtonKind(CommandButtonKind.ACTION_ONLY);
		RibbonUIManager.getInstance().bind(
				Datatreat.REMOVE_DATATREAT_TRANSFORM_TABLE, remove);
