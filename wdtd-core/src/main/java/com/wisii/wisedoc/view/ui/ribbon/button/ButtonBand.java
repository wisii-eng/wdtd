...
package com.wisii.wisedoc.view.ui.ribbon.button;
...
public class ButtonBand implements WiseBand
{

	@Override
	public JRibbonBand getBand()
	{
		JRibbonBand visibleGroupBand = new JRibbonBand(RibbonUIText.BUTTONS,
				MediaResource.getResizableIcon("09379.ico"), null);

		JCommandButton currentinline = new JCommandButton(RibbonUIText.SET,
		...
		RibbonUIManager.getInstance()
				.bind(Datatreat.SET_BUTTONS, currentinline);
		visibleGroupBand.addCommandButton(currentinline,
				RibbonElementPriority.TOP);

		JCommandButton remove = new JCommandButton(RibbonUIText.CLEAR,
				MediaResource.getResizableIcon("00906.ico"));
		remove.setCommandButtonKind(CommandButtonKind.ACTION_ONLY);
		RibbonUIManager.getInstance().bind(Datatreat.REMOVE_BUTTONS, remove);
