...
public class ParagraphKeepBreakPanel extends JPanel
{
	...
	// String next_column = UiText.PARAGRAPH_NEXT_COLUMN;
	...
	private void initialize()
	{
		...
		keepPanel.setBorder(new TitledBorder(new LineBorder(Color.GRAY),
				UiText.PARAGRAPH_PAGE_AND_COLUMN_LABEL, TitledBorder.LEFT,
				TitledBorder.TOP));

		JPanel widowsOrphans = new JPanel();

		widowsOrphans.setLayout(new GridLayout(1, 2));
		widowsOrphans.setPreferredSize(new Dimension(450, 60));

		JPanel widowspanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
		JPanel orphanspanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
		...
		widowspanel.add(new JLabel(UiText.PARAGRAPH_LAST_LINE_MIN_NUMBER));
		widowspanel.add(widows);
		orphanspanel.add(new JLabel(UiText.PARAGRAPH_FIRST_LINE_MIN_NUMBER));
		orphanspanel.add(orphans);
		widowsOrphans.add(widowspanel);
		widowsOrphans.add(orphanspanel);
		widowsOrphans.setBorder(new TitledBorder(new LineBorder(Color.GRAY),
				UiText.PARAGRAPH_WIDOWS_ORPHAN, TitledBorder.LEFT,
				TitledBorder.TOP));

		JPanel spanpanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
		final JCheckBox spancheckbox = new JCheckBox(UiText.SPAN_ALL);
		spanpanel.add(spancheckbox);
		Object spanpr = getAttributeValue(Constants.PR_SPAN);
		if (spanpr != null)
		{
			if (spanpr instanceof EnumProperty)
			{
				EnumProperty enumpspan = (EnumProperty) spanpr;
				int spanvalue = enumpspan.getEnum();
				spancheckbox.setSelected(spanvalue == Constants.EN_ALL);
			}
		}
		spancheckbox.addChangeListener(new ChangeListener()
		{
			...
			public void stateChanged(ChangeEvent e)
			{
				...
				{
					setAttribute(Constants.PR_SPAN, new EnumProperty(
							Constants.EN_ALL, ""));
				} else
				{
					setAttribute(Constants.PR_SPAN, new EnumProperty(
		...
		spanpanel.setBorder(new TitledBorder(new LineBorder(Color.GRAY),
				UiText.SPAN_ALL, TitledBorder.LEFT, TitledBorder.TOP));
		this.add(breakPanel);
		this.add(keepPanel);
	...
	public EnumProperty getEnumProperty(String item)
	{
		...
		// else if (item.equalsIgnoreCase(next_column))
		// {
		// value = Constants.EN_COLUMN;
		// }
	...
	public String getItem(int value)
	{
		...
		// else if (value == Constants.EN_COLUMN)
		// {
		// item = next_column;
		// }
