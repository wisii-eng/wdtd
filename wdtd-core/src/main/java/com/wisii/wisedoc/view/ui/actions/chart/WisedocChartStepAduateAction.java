 ...
 */
/**
 * @WisedocChartStepAduateAction.java
 * 汇智互联版权所有，未经许可，不得使用
 */
 ...
 */
public class WisedocChartStepAduateAction extends Actions {
	@Override
	public void doAction(final ActionEvent e) {
		if (e.getSource() instanceof WiseSpinner) {
			...
			setFOProperty(Constants.PR_RANGEAXIS_UNITINCREMENT, dValue);
		}
	}
	...
	protected void uiStateChange(final UIStateChangeEvent evt) {
		...
		if (hasPropertyKey(Constants.PR_RANGEAXIS_UNITINCREMENT)) {
			if (uiComponent instanceof WiseSpinner)
			{
				final WiseSpinner ui = (WiseSpinner) uiComponent;
				Object uinc = evt.getNewValue();
				if (uinc instanceof Double)
				{
					ui.initValue(uinc);
				} else
				{
	...
	public boolean isAvailable() {
		...
		return (prop.getEnum() != Constants.EN_PIECHART) && chart.getAttribute(Constants.PR_RANGEAXIS_UNITINCREMENT) != null;
	}
}
