 ...
 */
public class SvgGraphicFontSizeAction extends Actions {
...
//						System.out.println("请输入正确的数字");
			...
			//设置字号到属性中
			if (fontSize != -1) {
				setFOProperty(Constants.PR_FONT_SIZE,
	...
	public void uiStateChange(UIStateChangeEvent evt)
	{
		if (hasPropertyKey(Constants.PR_FONT_SIZE))
		{
			if (uiComponent instanceof WiseCombobox)
			{
				...
				if (fs instanceof FixedLength)
				{
					FixedLength value = (FixedLength) fs;
					ui.InitValue(value.toString());
				} else
				{
					ui.InitValue("");
				}
	...
	public void setDefaultState(ActionCommandType actionCommand) {
		...
		if (uiComponent instanceof WiseCombobox) {
			WiseCombobox ui = (WiseCombobox) uiComponent;
			ui.InitValue(UiText.FONT_SIZE_LIST[10]);
		}
	}
