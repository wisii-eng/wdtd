...
public class SetLinefedTreatmentAction extends Actions
{
	...
	protected void uiStateChange(UIStateChangeEvent evt)
	{
		...
		if (hasPropertyKey(Constants.PR_LINEFEED_TREATMENT))
		{
	...
	private void setProperty(int value)
	{
		...
		{
			result = Constants.EN_TREAT_AS_SPACE;
		}

		((WiseDocDocument)getCurrentDocument()).setAttribute(Constants.PR_LINEFEED_TREATMENT,
				result,true);
	}
	...
	public int getIndex(final int value)
	{
		...
		{
			result = 1;
		} else if (value == Constants.EN_TREAT_AS_SPACE)
		{
			result = 2;
		}
	...
	{
		init((Integer) getCurrentDocument().getAttribute(Constants.PR_LINEFEED_TREATMENT));
