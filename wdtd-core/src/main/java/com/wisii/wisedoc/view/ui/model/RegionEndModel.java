 ...
 * 
 <table width="609" border="1">
  ...
  <tr>
    ...
    <td>右区域的referenceOrientation</td>
    <td>0</td>
  </tr>
  <tr>
    <td>8</td>
...
public class RegionEndModel {
	...
	private RegionEndModel regionModel = this;
	
	public RegionEnd getRegionEnd(){
		if (regionModel == null) {
			return null;
		} else {
			return new RegionEnd(getExtent(), 
	...
	private CommonBorderPaddingBackground getRegionEndBackground(){
	...
	public static class Builder {
		...
		private void getCurrentProperty(){
			...
			if (currnetValue != null) {
				if (currnetValue instanceof SimplePageMaster) {
					...
					RegionEnd regionEnd = (RegionEnd) spm.getRegion(Constants.FO_REGION_END);
					
					if (regionEnd != null) {
						this.extent = regionEnd.getExtent();
						this.displayAlign = regionEnd.getDisplayAlign();
						this.overFlow = regionEnd.getOverflow();
						this.referenceOrientation = regionEnd.getRegionReferenceOrientation();
						this.regionName = regionEnd.getRegionName();
						this.writingMode = regionEnd.getRegionWritingMode();
						this.commonBorderPaddingBackground = regionEnd.getCommonBorderPaddingBackground();
						
						this.bodyBackgroundColor = regionEnd.getCommonBorderPaddingBackground().getBackgroundColor();
						this.bodyBackgroundImage = regionEnd.getCommonBorderPaddingBackground().getBackgroundImage();
						 this.backgroundImageLayer = regionEnd.getCommonBorderPaddingBackground().getBackgroundImageLayer();
						this.bodyBackgroundImageRepeat = new EnumProperty(regionEnd.getCommonBorderPaddingBackground().getBackgroundRepeat(),"");
						this.bodyBackgroundPositionHorizontal = regionEnd.getCommonBorderPaddingBackground().getBackgroundPositionHorizontal();
						this.bodyBackgroundPositionVertical = regionEnd.getCommonBorderPaddingBackground().getBackgroundPositionVertical();
					}
				}
			}
		}
		
		private void getRegionEndProperty(RegionEnd regionEnd){
			this.extent = regionEnd.getExtent();
			this.displayAlign = regionEnd.getDisplayAlign();
			this.overFlow = regionEnd.getOverflow();
			this.referenceOrientation = regionEnd.getRegionReferenceOrientation();
		 ...
		 * @param type
		 */
		public Builder defaultRegionEnd(){
			setDefault();
			return this;
		}
		...
		public Builder regionEnd(RegionEnd regionEnd){
			if (regionEnd != null) {
				setDefault();
				getRegionEndProperty(regionEnd);
				return this;
			} else {
				this.builder = null;
				return this;
		 ...
		 * @return
		 */
		public RegionEndModel build(){
			if (this.builder == null) {
				return new RegionEndModel(null);
			} else {
				return new RegionEndModel(this);
			}
		}
	...
	private RegionEndModel(Builder builder) {
		if (builder == null) {
