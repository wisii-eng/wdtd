...
public class RegionBeforeWritingModeAction extends
		DefaultSimplePageMasterActions
{
	...
	private void setProperty(int index)
	{
		...
		{
			case 0:
				current = setRegionBeforeWritingMode(Constants.EN_LR_TB);
				break;
			case 1:
				current = setRegionBeforeWritingMode(Constants.EN_RL_TB);
				break;
			case 2:
				current = setRegionBeforeWritingMode(Constants.EN_TB_RL);
				break;
			case 3:
				// FOP中没有bt-rl
						...
						&& list.get(0) instanceof JCommandMenuButton)
				{
					...
					if (value != null)
					{
						int trValue = value.getRegion(
								Constants.FO_REGION_BEFORE).getWritingMode();
