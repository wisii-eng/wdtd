 ...
 * @version 2009/01/14
 */
public class TableCellDisplayAlignAction extends Actions {
	...
	private void setProperty(int index){
		...
		case 0:
			//letter
			setFOProperty(Constants.PR_DISPLAY_ALIGN, new EnumProperty(Constants.EN_BEFORE, ""));
			break;
		case 1:
			//Tabloid
			...
			if (value instanceof Integer) {
				...
				if (value.equals(Constants.NULLOBJECT)) {
					...
					if (trValue == Constants.EN_BEFORE) {
						...
						((JCommandMenuButton)list.get(1)).getActionModel().setSelected(false);
						((JCommandMenuButton)list.get(2)).getActionModel().setSelected(false);
					} else if (trValue == Constants.EN_CENTER){
						((JCommandMenuButton)list.get(0)).getActionModel().setSelected(false);
