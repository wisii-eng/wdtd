...
public class RegionStartExtentAction extends DefaultSimplePageMasterActions
{
	...
	public void doAction(ActionEvent e)
	{
		if (e.getSource() instanceof FixedLengthSpinner)
		{
			...
			SimplePageMaster current = setRegionStartExtent(jb.getValue());
	...
	protected void uiStateChange(UIStateChangeEvent evt)
	{
				...
				|| hasPropertyKey(Constants.PR_CURRENT_SIMPLE_PAGE_MASTER))
		{
			...
			if (value instanceof SimplePageMaster)
			{
				...
				if (extent != null)
				{
					if (UiUtil.hasRegionStart(simValue))
					{
						FixedLength temp = (FixedLength) ((RegionStart) simValue
		...
		if (actionCommand == ActionCommandType.RIBBON_ACTION)
		{
			...
			if (RibbonUIModel.getCurrentPropertiesByType().get(this.actionType) != null)
			{
				...
				if (value != null)
				{
					if (extent != null)
					{
						if (UiUtil.hasRegionStart(value))
						{
							FixedLength temp = (FixedLength) ((RegionStart) value
									.getRegion(Constants.FO_REGION_START))
									.getExtent();
							extent.initValue(temp);
						}
