 ...
 */
/**
 * @WisedocChartStartDegreeAction.java
 * 汇智互联版权所有，未经许可，不得使用
 */
 ...
 */
public class WisedocChartStartDegreeAction extends Actions {
	@Override
	public void doAction(final ActionEvent e) {
		if (e.getSource() instanceof WiseSpinner) {
			...
			setFOProperty(Constants.PR_PIECHART_STARTANGLE, dValue);
		}
	}
	...
	protected void uiStateChange(final UIStateChangeEvent evt)
	{
		...
		if (hasPropertyKey(Constants.PR_PIECHART_STARTANGLE))
		{
			if (uiComponent instanceof WiseSpinner)
			{
				final WiseSpinner ui = (WiseSpinner) uiComponent;
				Object startangle = evt.getNewValue();
				if (startangle instanceof Integer)
				{
					ui.initValue(startangle);
				} else
				{
					ui.initValue(0);
				}
