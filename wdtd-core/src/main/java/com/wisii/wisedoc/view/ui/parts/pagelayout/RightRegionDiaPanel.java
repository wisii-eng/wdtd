 ...
 */
public class RightRegionDiaPanel extends JPanel implements UpdateSPMProperty {
...
//					.addComponent(checkBox)
	...
	private Map<Integer, Object> getRegionBodyBgProperties(){
		...
		temp.put(Constants.PR_BACKGROUND_COLOR, spmm.getRegionEndModel().getBodyBackgroundColor());
		temp.put(Constants.PR_BACKGROUND_IMAGE, spmm.getRegionEndModel().getBodyBackgroundImage());
		temp.put(Constants.PR_BACKGROUND_REPEAT, spmm.getRegionEndModel().getBodyBackgroundImageRepeat());
		temp.put(Constants.PR_BACKGROUND_POSITION_HORIZONTAL, spmm.getRegionEndModel().getBodyBackgroundPositionHorizontal());
		temp.put(Constants.PR_BACKGROUND_POSITION_VERTICAL, spmm.getRegionEndModel().getBodyBackgroundPositionVertical());
	...
	private class RefOrientationAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() instanceof JComboBox) {
				...
				case 0:
					spmm.getRegionEndModel().setReferenceOrientation(0);
					break;
				case 1:
					spmm.getRegionEndModel().setReferenceOrientation(90);
					break;
				case 2:
					spmm.getRegionEndModel().setReferenceOrientation(180);
					break;
				case 3:
					spmm.getRegionEndModel().setReferenceOrientation(270);
					break;
				case 4:
					spmm.getRegionEndModel().setReferenceOrientation(-90);
					break;
		...
		public void setDefaultState(){
			...
			switch (spmm.getRegionEndModel().getReferenceOrientation()) {
			case 0:
	...
	private class WritingModeAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() instanceof JComboBox) {
				...
				case 0:
					spmm.getRegionEndModel().setWritingMode(Constants.EN_LR_TB);
					break;
				case 1:
					spmm.getRegionEndModel().setWritingMode(Constants.EN_RL_TB);
					break;
				case 2:
					spmm.getRegionEndModel().setWritingMode(Constants.EN_TB_RL);
					break;
				case 3:
					//FOP中没有bt-rl
					spmm.getRegionEndModel().setWritingMode(-1);
					break;
//				case 4:
//					//也没有lr-bt
...
//					break;
		...
		public void setDefaultState(){
			...
			int trValue = spmm.getRegionEndModel().getWritingMode();
	...
	private class DisplayAlignAction implements ActionListener {
		...
		private void setProperty(int index){
			switch (index) {
			case 0:
				spmm.getRegionEndModel().setDisplayAlign(Constants.EN_TOP);
				break;
			case 1:
				spmm.getRegionEndModel().setDisplayAlign(Constants.EN_CENTER);
				break;
			case 2:
				spmm.getRegionEndModel().setDisplayAlign(Constants.EN_AFTER);
				break;
		...
		public void setDefaultState(){
			...
			int trValue = spmm.getRegionEndModel().getDisplayAlign();
	...
	private class OverflowAction implements ActionListener {
		...
		private void setProperty(int index){
			switch (index) {
			case 0:
				spmm.getRegionEndModel().setOverflow(Constants.EN_AUTO);
				break;
			case 1:
				spmm.getRegionEndModel().setOverflow(Constants.EN_VISIBLE);
				break;
			case 2:
				spmm.getRegionEndModel().setOverflow(Constants.EN_HIDDEN);
				break;
			case 3:
				spmm.getRegionEndModel().setOverflow(Constants.EN_SCROLL);
				break;
		...
		public void setDefaultState(){
			...
			int trValue = spmm.getRegionEndModel().getOverflow();
	...
	private class ExtentAction implements ActionListener {
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			spmm.getRegionEndModel().setExtent(spinner.getValue());
			//为了不出现版心区域和页眉部分互相覆盖的情况，需要同时把版心区域的margin设置成和页眉高度一样的值
			//因此版心区域的margin的更新还是一个bug
			spmm.getRegionBodyModel().setMarginRight(spinner.getValue());
		}
		...
		public void setDefaultState(){
			...
			spinner.initValue(spmm.getRegionEndModel().getExtent());
		}
	}
}
