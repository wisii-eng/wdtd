...
import com.wisii.wisedoc.view.ui.SQLTreeDialog;
import com.wisii.wisedoc.view.ui.parts.dialogs.BindingSetingDialog;

public class LoadSQLStructureAction extends BaseAction
{
	 ...
	 *                ,将发生什么异常}
	 */
	public LoadSQLStructureAction(String name)
	{
		super(name);
	}
	 ...
	 *                ,将发生什么异常}
	 */
	public LoadSQLStructureAction(String name, Icon icon)
	{
		super(name, icon);
	}
	...
	{
		DataStructureTreeModel oldmodel = SystemManager.getCurruentDocument()
				.getDataStructure();
		Setting setold = null;
		if (oldmodel != null)
		{
			setold = oldmodel.getDbsetting();
		}
		SQLTreeDialog dia = new SQLTreeDialog(setold);
		if (dia.showDialog() == DialogResult.OK)
		{
			DataStructureTreeModel model = dia.getDataStructureModel();
			if (model != null)
			{
			...
			} else
			{
				WiseDocOptionPane.showMessageDialog(SystemManager
						.getMainframe(), "数据结构错误，请检查查询sql语句！");
			}
		}
