 ...
 */
public class SetContentHeightAction extends Actions {
	...
	public void doAction(ActionEvent e)
	{
		if (e.getSource() instanceof FixedLengthSpinner)
		{
			...
			if (actionCommandType == ActionCommandType.DYNAMIC_ACTION) {
				PicturePropertyPanel.getNewMap().put(Constants.PR_CONTENT_HEIGHT, newlen);
			} else {
				setFOProperty(Constants.PR_CONTENT_HEIGHT, newlen);
			}
		}
	}
	...
	{
		if (hasPropertyKey(Constants.PR_CONTENT_HEIGHT))
		{
	...
	protected void setDifferentPropertyState(UIStateChangeEvent evt)
	{
		...
		if (uiComponent instanceof FixedLengthSpinner)
		{
			...
			oldlength = (Length) firstelement
					.getAttribute(Constants.PR_CONTENT_HEIGHT);
			FixedLength uivalue = null;
			if (oldlength instanceof EnumLength)
			{
				Length length = ((EnumLength) oldlength).getFixLength();
	...
	public boolean isAvailable()
	{
		...
		if (temp != null)
		{
			Length height = (Length) temp.get(Constants.PR_CONTENT_HEIGHT);
			if (height != null)
			{
				return height.getEnum() != Constants.EN_AUTO;
			}
		}
		return false;
	}
