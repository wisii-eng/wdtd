 ...
 */
public class HeaderDiaPanel extends JPanel implements UpdateSPMProperty {
...
//		comboBox_3 = new JComboBox(new DefaultComboBoxModel(UiText.COMMON_MEASUREMENT_LIST));
		...
		comboBox_4 = new JComboBox(new DefaultComboBoxModel(UiText.PAGE_BODY_CONTENT_OVERFLOW_LIST));
		
		comboBox_4.addActionListener(oa);
		oa.setDefaultState();
		
		checkBox = new JCheckBox();
		checkBox.setText("优先显示页眉");
...
//						.addComponent(comboBox_3, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE)
	...
	private Map<Integer, Object> getRegionBodyBgProperties(){
		...
		temp.put(Constants.PR_BACKGROUND_COLOR, spmm.getRegionBeforeModel().getBodyBackgroundColor());
		temp.put(Constants.PR_BACKGROUND_IMAGE, spmm.getRegionBeforeModel().getBodyBackgroundImage());
		temp.put(Constants.PR_BACKGROUND_REPEAT, spmm.getRegionBeforeModel().getBodyBackgroundImageRepeat());
		temp.put(Constants.PR_BACKGROUND_POSITION_HORIZONTAL, spmm.getRegionBeforeModel().getBodyBackgroundPositionHorizontal());
		temp.put(Constants.PR_BACKGROUND_POSITION_VERTICAL, spmm.getRegionBeforeModel().getBodyBackgroundPositionVertical());
	...
	private class RefOrientationAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() instanceof JComboBox) {
				...
				case 0:
					spmm.getRegionBeforeModel().setReferenceOrientation(0);
					break;
				case 1:
					spmm.getRegionBeforeModel().setReferenceOrientation(90);
					break;
				case 2:
					spmm.getRegionBeforeModel().setReferenceOrientation(180);
					break;
				case 3:
					spmm.getRegionBeforeModel().setReferenceOrientation(270);
					break;
				case 4:
					spmm.getRegionBeforeModel().setReferenceOrientation(-90);
					break;
		...
		public void setDefaultState(){
			...
			switch (spmm.getRegionBeforeModel().getReferenceOrientation()) {
			case 0:
	...
	private class WritingModeAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() instanceof JComboBox) {
				...
				case 0:
					spmm.getRegionBeforeModel().setWritingMode(Constants.EN_LR_TB);
					break;
				case 1:
					spmm.getRegionBeforeModel().setWritingMode(Constants.EN_RL_TB);
					break;
				case 2:
					spmm.getRegionBeforeModel().setWritingMode(Constants.EN_TB_RL);
					break;
				case 3:
					//FOP中没有bt-rl
					spmm.getRegionBeforeModel().setWritingMode(-1);
					break;
//				case 4:
//					//也没有lr-bt
...
//					break;
		...
		public void setDefaultState(){
			...
			int trValue = spmm.getRegionBeforeModel().getWritingMode();
	...
	private class DisplayAlignAction implements ActionListener {
		...
		private void setProperty(int index){
			switch (index) {
			case 0:
				spmm.getRegionBeforeModel().setDisplayAlign(Constants.EN_TOP);
				break;
			case 1:
				spmm.getRegionBeforeModel().setDisplayAlign(Constants.EN_CENTER);
				break;
			case 2:
				spmm.getRegionBeforeModel().setDisplayAlign(Constants.EN_AFTER);
				break;
		...
		public void setDefaultState(){
			...
			int trValue = spmm.getRegionBeforeModel().getDisplayAlign();
	...
	private class OverflowAction implements ActionListener {
		...
		private void setProperty(int index){
			switch (index) {
			case 0:
				spmm.getRegionBeforeModel().setOverflow(Constants.EN_AUTO);
				break;
			case 1:
				spmm.getRegionBeforeModel().setOverflow(Constants.EN_VISIBLE);
				break;
			case 2:
				spmm.getRegionBeforeModel().setOverflow(Constants.EN_HIDDEN);
				break;
			case 3:
				spmm.getRegionBeforeModel().setOverflow(Constants.EN_SCROLL);
				break;
		...
		public void setDefaultState(){
			...
			int trValue = spmm.getRegionBeforeModel().getOverflow();
	...
	private class ExtentAction implements ActionListener {
		...
		@Override
		public void actionPerformed(ActionEvent e) {
			spmm.getRegionBeforeModel().setExtent(spinner.getValue());
			//为了不出现版心区域和页眉部分互相覆盖的情况，需要同时把版心区域的margin设置成和页眉高度一样的值
			//因此版心区域的margin的更新还是一个bug
			spmm.getRegionBodyModel().setMarginTop(spinner.getValue());
		}
		...
		public void setDefaultState(){
			...
			spinner.initValue(spmm.getRegionBeforeModel().getExtent());
		}
	}
	...
	private class PrecedenceAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (checkBox.isSelected()) {
				spmm.getRegionBeforeModel().setPrecedence(Constants.EN_TRUE);
			} else {
				spmm.getRegionBeforeModel().setPrecedence(Constants.EN_FALSE);
			}
		}
		...
		public void setDefaultState(){
			...
			int temp = spmm.getRegionBeforeModel().getPrecedence();
