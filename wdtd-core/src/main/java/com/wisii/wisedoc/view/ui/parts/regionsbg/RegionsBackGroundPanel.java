 ...
 */
public class RegionsBackGroundPanel extends JPanel
{
	private JTextField pathlabel = new JTextField();
	// 背景色选择框
	private ColorComboBox colorbox;
	// 背景颜色层标签
	JLabel colorlayerlabel = new JLabel("层:");
	// 背景色的层
	private LayerComboBox colorlayerbox = new LayerComboBox();
	// 背景图片的层标签
	JLabel imagelayerlabel = new JLabel("层:");
	// 背景图片的层
	LayerComboBox imagelayerbox = new LayerComboBox();
	// 平铺方式标签
	JLabel repeatlabel = new JLabel("重复设置:");
	// 平铺方式下拉选择框
	WiseCombobox repeatbox = new WiseCombobox(new String[]
	{ "不重复", "横向纵向均重复", "横向重复", "纵向重复" });
	Map<Integer, Object> attributemap = new HashMap<Integer, Object>();
	// 横向位置标签
	JLabel poshlabel = new JLabel("横向位置:");
	// 横向位置下拉选择框
	WiseCombobox poshbox = new WiseCombobox(new String[]
	{ "左对齐", "居中", "右对齐", "固定值" });
	// 横向固定值设置控件
	FixedLengthSpinner poshspiner = new FixedLengthSpinner();
	// 纵向位置标签
	JLabel posvlabel = new JLabel("纵向位置:");
	// 纵向位置下拉选择框
	WiseCombobox posvbox = new WiseCombobox(new String[]
	{ "上对齐", "居中", "下对齐", "固定值" });
	// 静态图片路径选择按钮
	JButton pathbutton = new JButton("静态路径");
	// 动态图片选择按钮
	JCommandButton bandingpathbutton = new JCommandButton("", MediaResource
			.getResizableIcon("00146.ico"));
	// 固定纵向位置设置控件
	FixedLengthSpinner posvspiner = new FixedLengthSpinner();

	public RegionsBackGroundPanel(Map<Integer, Object> atts, ActionID actionID)
	{
...
//		如果类型为空的话，则所有设置控件均不可用
		boolean isallenable = actionID!= null;
//		背景图片设置相关控件是否可用
...
//		}
		...
		colorpanel.setBorder(new TitledBorder("填充颜色"));
		try
		{
		...
		{
			LogUtil.debug("颜色面板初始化不成功", e);
		}
		JPanel imagepanel = new JPanel(new VerticalFlowLayout());
		imagepanel.setBorder(new TitledBorder("填充图案"));
		...
		bandingpathbutton.setPopupRichTooltip(new RichTooltip("动态路径",
				"选择数据节点作为路径"));
		bandingpathbutton
				.setCommandButtonKind(JCommandButton.CommandButtonKind.POPUP_ONLY);
		bandingpathbutton.setEnabled(isimagesetenable&&SystemManager.getCurruentDocument()
				.getDataStructure() != null);
