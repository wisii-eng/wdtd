 ...
 * 创建日期：2009-7-13
 */
public class InputFilterAction extends AbstractEditAction
{
	public void doAction(ActionEvent e)
	{
		if (e.getSource() instanceof WiseTextField)
		{
			...
			{
				setFOProperty(Constants.PR_INPUT_FILTER, null);
				setFOProperty(Constants.PR_INPUT_FILTERMSG, null);
			} else
			{
				setFOProperty(Constants.PR_INPUT_FILTER, text.trim());
			}
		}
	...
	protected void uiStateChange(UIStateChangeEvent evt)
	{
		...
		if (uiComponent instanceof WiseTextField
				&& hasPropertyKey(Constants.PR_INPUT_FILTER))
		{
	...
	protected void setDifferentPropertyState(UIStateChangeEvent evt)
	{
		...
		if (evt.hasPropertyKey(Constants.PR_INPUT_FILTER))
		{
			if (uiComponent instanceof WiseTextField)
			{
				WiseTextField textfield = (WiseTextField) uiComponent;
