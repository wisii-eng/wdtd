 ...
 */
/**
 * @WisedocChartShowBaseLineAction.java
 * 汇智互联版权所有，未经许可，不得使用
 */
...
/**
 * 类功能说明：用于控制统计图，是否显示零基线
 * @author 李晓光
 * @version 1.0 2009-5-22
 */
public class WisedocChartShowBaseLineAction extends Actions {
	@Override
	public void doAction(final ActionEvent e) {
		if (e.getSource() instanceof JCheckBox) {
			...
			setFOProperty(Constants.PR_ZERORANGELINE_VISABLE, cond);
		}
	}
	...
	protected void uiStateChange(final UIStateChangeEvent evt) {
		...
		if (hasPropertyKey(Constants.PR_ZERORANGELINE_VISABLE)) {
	...
	protected void setDifferentPropertyState(final UIStateChangeEvent evt) {
		...
		if (evt.hasPropertyKey(Constants.PR_ZERORANGELINE_VISABLE)) {
			if (uiComponent instanceof JCheckBox) {
				final JCheckBox ui = (JCheckBox) uiComponent;
				ui.setSelected(false);
			}
