 ...
 */
/**
 * @WisedocChartMinAduateAction.java
 * 汇智互联版权所有，未经许可，不得使用
 */
 ...
 */
public class WisedocChartMinAduateAction extends Actions {
	@Override
	public void doAction(final ActionEvent e) {
		if (e.getSource() instanceof WiseSpinner) {
			...
			setFOProperty(Constants.PR_RANGEAXIS_MINUNIT, dValue);
		}
	}
	...
	protected void uiStateChange(final UIStateChangeEvent evt)
	{
		...
		if (hasPropertyKey(Constants.PR_RANGEAXIS_MINUNIT))
		{
			if (uiComponent instanceof WiseSpinner)
			{
				final WiseSpinner ui = (WiseSpinner) uiComponent;
				Object minunit = evt.getNewValue();
				if (minunit instanceof Double)
				{
					ui.initValue(minunit);
				} else
				{
					ui.initValue(0.0);
				}
	...
	public boolean isAvailable() {
		...
		return (prop.getEnum() != Constants.EN_PIECHART) && chart.getAttribute(Constants.PR_RANGEAXIS_MINUNIT) != null;
	}
}
