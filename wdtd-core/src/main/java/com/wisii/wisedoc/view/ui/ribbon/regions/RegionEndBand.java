...
import static com.wisii.wisedoc.view.ui.text.RibbonUIText.REGION_END_DISPLAY_ALIGN_BUTTON;
import static com.wisii.wisedoc.view.ui.text.RibbonUIText.REGION_END_OVER_FLOW_BUTTON;
import static com.wisii.wisedoc.view.ui.text.RibbonUIText.REGION_END_OVER_FLOW_BUTTON_DESCRIPTION;
import static com.wisii.wisedoc.view.ui.text.RibbonUIText.REGION_END_OVER_FLOW_BUTTON_TITLE;
import static com.wisii.wisedoc.view.ui.text.RibbonUIText.REGION_END_REFERENCE_ORIENTATION_BUTTON_DESCRIPTION;
import static com.wisii.wisedoc.view.ui.text.RibbonUIText.REGION_END_REFERENCE_ORIENTATION_BUTTON_TITLE;
...
public class RegionEndBand {
	...
	public JRibbonBand getBand() {
		...
		referenceOrientation.setPopupRichTooltip(new RichTooltip(REGION_END_REFERENCE_ORIENTATION_BUTTON_TITLE, REGION_END_REFERENCE_ORIENTATION_BUTTON_DESCRIPTION));
		...
		//内容对齐方式
		JCommandButton displayAlign = new JCommandButton(REGION_END_DISPLAY_ALIGN_BUTTON, MediaResource.getResizableIcon("00664.ico"));
		...
		//溢出处理
		JCommandButton overflow = new JCommandButton(REGION_END_OVER_FLOW_BUTTON, MediaResource.getResizableIcon("09636.ico"));
		overflow.setPopupCallback(new Overflow());
		overflow.setPopupRichTooltip(new RichTooltip(REGION_END_OVER_FLOW_BUTTON_TITLE, REGION_END_OVER_FLOW_BUTTON_DESCRIPTION));
		...
		backgroundimage.setPopupCallback(new PopupPanelCallback() {
			public JPopupPanel getPopupPanel(JCommandButton arg0) {
				return new RegionsBackGroundImageList(RegionEnd.INSTANCE);
			}
		});
	...
	private class WritingMode extends JCommandPopupMenu	implements PopupPanelCallback {
		public WritingMode() {
			...
			RibbonUIManager.getInstance().bind(RegionEnd.WRITING_MODE_ACTION, lrtb, rltb, tbrl);
	...
	private class DisplayAlign extends JCommandPopupMenu implements PopupPanelCallback {
		...
		public DisplayAlign() {
			...
			RibbonUIManager.getInstance().bind(RegionEnd.DISPLAY_ALIGN_ACTION, before, center, after);
	...
	private class ReferenceOrientation extends JCommandPopupMenu implements PopupPanelCallback {
		...
		public ReferenceOrientation() {
			...
			RibbonUIManager.getInstance().bind(RegionEnd.REFERENCE_ORIENTATION_ACTION, jmb);
	...
	private class Overflow extends JCommandPopupMenu implements PopupPanelCallback {
		...
		public Overflow() {
			...
			RibbonUIManager.getInstance().bind(RegionEnd.OVERFLOW_ACTION, jmb);
		...
		public BackGroundColor() {
			try {
				...
				RibbonUIManager.getInstance().bind(
						RegionEnd.BACKGROUND_COLOR_ACTION, colorBox);
