 ...
 * 根据用户输入设置词间距
 * @author	闫舒寰
 * @version	0.1 2008/10/8
 */
public class FontWordSpaceInputAction extends Actions implements ChangeListener{
	...
	private void getComponents(){
		...
		Set<List<Object>> comSet = RibbonUIManager.getUIComponents().get(Font.WORD_SPACE_INPUT_ACTION).get(actionCommandType);
	...
	public void doAction(ActionEvent e) {
		...
		// 设置正确的属性类型和属性值
		FontPropertyModel.setFOProperty(Constants.PR_WORD_SPACING, space);
	}
	...
	public void setDefaultState(ActionCommandType actionCommand) {
		...
		if (actionCommand == ActionCommandType.DIALOG_ACTION) {
			...
			getComponents();
			Object value = InitialManager.getInitialValue(Constants.PR_WORD_SPACING, null);
			Map<Integer,Object> atts = RibbonUIModel
					.getCurrentPropertiesByType().get(
							this.actionType);
			if (atts != null&&atts.containsKey(Constants.PR_WORD_SPACING))
			{
				value = atts.get(Constants.PR_WORD_SPACING);
			}
			...
			{
				FixedLengthSpinner spinnerUI = (FixedLengthSpinner) this.value;
		...
		if (actionCommand == ActionCommandType.DYNAMIC_ACTION) {
			...
			Object currnetValue =  FontPropertyPanel.getInitialPropertise().get(Constants.PR_WORD_SPACING);
