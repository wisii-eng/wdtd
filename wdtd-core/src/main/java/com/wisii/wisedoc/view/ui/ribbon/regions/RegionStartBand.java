...
import static com.wisii.wisedoc.view.ui.text.RibbonUIText.REGION_START_DISPLAY_ALIGN_BUTTON;
import static com.wisii.wisedoc.view.ui.text.RibbonUIText.REGION_START_OVER_FLOW_BUTTON;
import static com.wisii.wisedoc.view.ui.text.RibbonUIText.REGION_START_OVER_FLOW_BUTTON_DESCRIPTION;
import static com.wisii.wisedoc.view.ui.text.RibbonUIText.REGION_START_OVER_FLOW_BUTTON_TITLE;
import static com.wisii.wisedoc.view.ui.text.RibbonUIText.REGION_START_REFERENCE_ORIENTATION_BUTTON_DESCRIPTION;
import static com.wisii.wisedoc.view.ui.text.RibbonUIText.REGION_START_REFERENCE_ORIENTATION_BUTTON_TITLE;
 ...
 */
public class RegionStartBand
{

	public JRibbonBand getBand()
	{

		JRibbonBand band = new JRibbonBand(RibbonUIText.WRITEMODE_REFERENCEORIENTATION_ALIGN_OVERFLOW_BACKGROUND,
				MediaResource.getResizableIcon("09379.ico"), null);

		// 写作模式
		JCommandButton writingMode = new JCommandButton(
				RibbonUIText.TEXT_ORIENTATION_BUTTON_TITLE, MediaResource
		...
		// 鼠标浮动显示解释说明
		writingMode.setActionRichTooltip(new RichTooltip(
		...
		// 文字方向
		JCommandButton referenceOrientation = new JCommandButton(
				UiText.REFERENCE_ORIENTATION, MediaResource
						.getResizableIcon("00405.ico"));
		referenceOrientation.setPopupCallback(new ReferenceOrientation());
		referenceOrientation.setPopupRichTooltip(new RichTooltip(
				REGION_START_REFERENCE_ORIENTATION_BUTTON_TITLE,
				REGION_START_REFERENCE_ORIENTATION_BUTTON_DESCRIPTION));
		referenceOrientation.setCommandButtonKind(CommandButtonKind.POPUP_ONLY);
		band.addCommandButton(referenceOrientation,
				RibbonElementPriority.MEDIUM);

		// 内容对齐方式
		JCommandButton displayAlign = new JCommandButton(
				REGION_START_DISPLAY_ALIGN_BUTTON, MediaResource
		...
		// 溢出处理
		JCommandButton overflow = new JCommandButton(
				REGION_START_OVER_FLOW_BUTTON, MediaResource
						.getResizableIcon("09636.ico"));
		overflow.setPopupCallback(new Overflow());
		overflow.setPopupRichTooltip(new RichTooltip(
				REGION_START_OVER_FLOW_BUTTON_TITLE,
				REGION_START_OVER_FLOW_BUTTON_DESCRIPTION));
		overflow.setCommandButtonKind(CommandButtonKind.POPUP_ONLY);
		band.addCommandButton(overflow, RibbonElementPriority.MEDIUM);

		// 背景图片按钮
		JCommandButton backgroundimage = new JCommandButton(
				RibbonUIText.BODY_BACKGROUND_PICTURE_BUTTON, MediaResource
						.getResizableIcon("00931.ico"));
		backgroundimage.setPopupRichTooltip(new RichTooltip(
				RibbonUIText.BODY_BACKGROUND_PICTURE_BUTTON_TITLE,
		...
		backgroundimage.setPopupCallback(new PopupPanelCallback()
		{
			...
			{
				return new RegionsBackGroundImageList(RegionStart.INSTANCE);
			}
		});
		...
		RibbonUIManager.getInstance().bind(Page.BODY_BACKGROUND_IMAGE_ACTION,
				backgroundimage, new Object());

		// 背景颜色按钮
		JCommandButton backgroundColor = new JCommandButton(
				RibbonUIText.BODY_BACKGROUND_COLOR_BUTTON, MediaResource
						.getResizableIcon("00417.ico"));
		backgroundColor.setPopupRichTooltip(new RichTooltip(
				RibbonUIText.BODY_BACKGROUND_COLOR_BUTTON_TITLE,
		...
		RibbonUIManager.getInstance().bind(Page.BODY_BACKGROUND_COLOR_ACTION,
				backgroundColor);
	 ...
	 * 写作方式下拉菜单
	 */
	private class WritingMode extends JCommandPopupMenu implements
			PopupPanelCallback
	{
		...
		public WritingMode()
		{
			...
			RibbonUIManager.getInstance().bind(RegionStart.WRITING_MODE_ACTION,
					lrtb, rltb, tbrl);
			...
			PopupPanelCallback
	{
		...
		public DisplayAlign()
		{
			...
			RibbonUIManager.getInstance().bind(
					RegionStart.DISPLAY_ALIGN_ACTION, before, center, after);
	 ...
	 * 文字方向下拉菜单
	 */
	private class ReferenceOrientation extends JCommandPopupMenu implements
			PopupPanelCallback
	{
		...
		public ReferenceOrientation()
		{
			...
			{
				jmb[i] = new JCommandMenuButton(
						UiText.PAGE_BODY_ORIENTATION_LIST[i],
						new EmptyResizableIcon(16));
			}

			RibbonUIManager.getInstance().bind(
					RegionStart.REFERENCE_ORIENTATION_ACTION, jmb);
	 ...
	 * 溢出处理下拉菜单
	 */
	private class Overflow extends JCommandPopupMenu implements
			PopupPanelCallback
	{
		...
		public Overflow()
		{
			...
			{
				jmb[i] = new JCommandMenuButton(
						UiText.PAGE_BODY_CONTENT_OVERFLOW_LIST[i], icons[i]);
			}

			RibbonUIManager.getInstance()
					.bind(RegionStart.OVERFLOW_ACTION, jmb);
	...
	private class BackGroundColor extends JCommandPopupMenu
	{
		...
		public BackGroundColor()
		{
			try
			{
				...
				RibbonUIManager.getInstance().bind(
						RegionStart.BACKGROUND_COLOR_ACTION, colorBox);
