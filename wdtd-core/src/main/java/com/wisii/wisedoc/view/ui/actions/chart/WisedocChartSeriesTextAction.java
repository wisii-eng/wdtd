 ...
 */
/**
 * @WisedocChartSeriesTextAction.java
 * 汇智互联版权所有，未经许可，不得使用
 */
 ...
 */
public class WisedocChartSeriesTextAction extends Actions
{
	...
	public void doAction(final ActionEvent e)
	{
		if (e.getSource() instanceof WisedocTextField)
		{
			...
			setFOProperty(Constants.PR_DOMAINAXIS_LABEL, text);
		}
	}
	...
	protected void uiStateChange(final UIStateChangeEvent evt)
	{
		...
		if (hasPropertyKey(Constants.PR_DOMAINAXIS_LABEL))
		{
		...
		{
			final WisedocTextField ui = (WisedocTextField) uiComponent;
			final Object obj = InitialManager.getInitialValue(
					Constants.PR_DOMAINAXIS_LABEL, null);
			String text = "";
			if (obj instanceof BarCodeText)
			{
				text = ((BarCodeText) obj).toString();
	...
	protected void setDifferentPropertyState(final UIStateChangeEvent evt)
	{
		...
		if (evt.hasPropertyKey(Constants.PR_DOMAINAXIS_LABEL))
		{
			if (uiComponent instanceof WisedocTextField)
			{
				final WisedocTextField ui = (WisedocTextField) uiComponent;
	...
	public boolean isAvailable()
	{
		...
		final EnumProperty prop = (EnumProperty) chart
				.getAttribute(Constants.PR_CHART_TYPE);
		if (prop == null)
		{
			return Boolean.FALSE;
		}
