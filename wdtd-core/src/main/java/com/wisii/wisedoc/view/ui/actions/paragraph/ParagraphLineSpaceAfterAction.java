...
public class ParagraphLineSpaceAfterAction extends Actions implements ChangeListener{
	...
	private void getComponents(){
		...
		Set<List<Object>> comSet = RibbonUIManager.getUIComponents().get(Paragraph.LINE_SPACE_AFTER_ACTION).get(ActionCommandType.DIALOG_ACTION);
	...
	private void setPorperty(){
		...
		// 设置正确的属性类型和属性值
		ParagraphPropertyModel.setFOProperty(Constants.PR_SPACE_AFTER, space);
	}
	...
	public void setDefaultState(ActionCommandType actionCommand) {
		...
		if (actionCommand == ActionCommandType.DIALOG_ACTION) {
			...
			Object currnetValue =  RibbonUIModel.getCurrentPropertiesByType().get(this.actionType).get(Constants.PR_SPACE_AFTER);
			double setValue = 0;
			String unit = null;
