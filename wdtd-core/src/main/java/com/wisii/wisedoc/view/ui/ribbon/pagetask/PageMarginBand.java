...
public class PageMarginBand
{

	public JRibbonBand getBand()
	{

		JRibbonBand bodyBand = new JRibbonBand(UiText.PAGE_MARGINS_LABEL,
				MediaResource.getResizableIcon("09379.ico"), null);

		// 版心边距
		MarginPanel bodyBorderSpacePanel = new MarginPanel(
				UiText.PAGE_MARGINS_LABEL);
		// bodyBand.addPanel(mp);
		// 以Ribbon组件对象包装当前组件，然后以流式排版排列
		/*
		 * JRibbonComponent bodyBorderSpacePanelWrapper = new
		 * JRibbonComponent(bodyBorderSpacePanel);
		 * bodyBand.addRibbonComponent(bodyBorderSpacePanelWrapper);
		 */

		bodyBand.startGroup(UiText.PAGE_MARGINS_LABEL);
		// bodyBand.startGroup();
		...
		RibbonUIManager.getInstance().bind(Page.MARGIN_ACTION,
				bodyBorderSpacePanel.getFixedlengthSpinners());
