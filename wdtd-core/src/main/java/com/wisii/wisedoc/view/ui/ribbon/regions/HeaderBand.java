...
public class HeaderBand {
	...
	public JRibbonBand getBand() {
		...
		referenceOrientation.setPopupRichTooltip(new RichTooltip(RibbonUIText.REGION_BEFORE_REFERENCE_ORIENTATION_BUTTON_TITLE,
				RibbonUIText.REGION_BEFORE_REFERENCE_ORIENTATION_BUTTON_DESCRIPTION));
		...
		//内容对齐方式
		JCommandButton displayAlign = new JCommandButton(RibbonUIText.BODY_DISPLAY_ALIGN_BUTTON, MediaResource.getResizableIcon("00664.ico"));
		...
		//溢出处理
		JCommandButton overflow = new JCommandButton(RibbonUIText.REGION_BEFORE_OVER_FLOW_BUTTON, MediaResource.getResizableIcon("09636.ico"));
		overflow.setPopupCallback(new Overflow());
		overflow.setPopupRichTooltip(new RichTooltip(RibbonUIText.REGION_BEFORE_OVER_FLOW_BUTTON_TITLE, RibbonUIText.REGION_BEFORE_OVER_FLOW_BUTTON_DESCRIPTION));
		...
		backgroundimage.setPopupCallback(new PopupPanelCallback() {
			public JPopupPanel getPopupPanel(JCommandButton arg0) {
				return new RegionsBackGroundImageList(RegionBefore.INSTANCE);
			}
		});
	...
	private class WritingMode extends JCommandPopupMenu	implements PopupPanelCallback {
		public WritingMode() {
			...
			RibbonUIManager.getInstance().bind(RegionBefore.WRITING_MODE_ACTION, lrtb, rltb, tbrl);
	...
	private class DisplayAlign extends JCommandPopupMenu implements PopupPanelCallback {
		...
		public DisplayAlign() {
			...
			RibbonUIManager.getInstance().bind(RegionBefore.DISPLAY_ALIGN_ACTION, before, center, after);
	...
	private class ReferenceOrientation extends JCommandPopupMenu implements PopupPanelCallback {
		...
		public ReferenceOrientation() {
			...
			RibbonUIManager.getInstance().bind(RegionBefore.REFERENCE_ORIENTATION_ACTION, jmb);
	...
	private class Overflow extends JCommandPopupMenu implements PopupPanelCallback {
		...
		public Overflow() {
			...
			RibbonUIManager.getInstance().bind(RegionBefore.OVERFLOW_ACTION, jmb);
		...
		public BackGroundColor() {
			try {
				...
				RibbonUIManager.getInstance().bind(
						RegionBefore.BACKGROUND_COLOR_ACTION, colorBox);
