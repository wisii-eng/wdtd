...
public class RegionAfterExtentAction extends DefaultSimplePageMasterActions
{
	...
	public void doAction(ActionEvent e)
	{
		if (e.getSource() instanceof FixedLengthSpinner)
		{
			...
			SimplePageMaster current = setRegionAfterExtent(jb.getValue());
				...
				|| hasPropertyKey(Constants.PR_CURRENT_SIMPLE_PAGE_MASTER))
		{
			...
			if (value instanceof SimplePageMaster)
			{
				...
				if (extent != null)
				{
					...
					{
						FixedLength temp = (FixedLength) ((com.wisii.wisedoc.document.attribute.RegionAfter) simValue
			...
			if (RibbonUIModel.getCurrentPropertiesByType().get(this.actionType) != null)
			{
				...
				if (value != null)
				{
					if (extent != null)
					{
						...
						{
							FixedLength temp = (FixedLength) ((com.wisii.wisedoc.document.attribute.RegionAfter) value
									.getRegion(Constants.FO_REGION_AFTER))
									.getExtent();
							extent.initValue(temp);
						}
