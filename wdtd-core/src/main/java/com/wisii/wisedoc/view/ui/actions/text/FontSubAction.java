 ...
 *
 */
public class FontSubAction extends Actions
{
	/**
	 * TODO
	 * 1、该动作的有效性覆盖范围不正确
	...
	{
		JCommandToggleButton superButton = (JCommandToggleButton) e.getSource();
		Object normalFontSize = RibbonUIModel.getCurrentPropertiesByType().get(this.actionType)
				.get(Constants.PR_FONT_SIZE);
		...
		} else
		{
			if (normalFontSize instanceof PercentLength)
			{
				...
				if (perlen.getBaseLength() instanceof LengthBase)
				{
					...
					temp.put(Constants.PR_FONT_SIZE, lenbase.getBaseLength());
					setFOProperties(temp);
				}
			}
	...
	protected void uiStateChange(UIStateChangeEvent evt)
	{
		...
		if (hasPropertyKey(Constants.PR_BASELINE_SHIFT))
		{
			if (uiComponent instanceof JCommandToggleButton)
			{
				...
				ui.getActionModel().setSelected(shiftvalue instanceof EnumLength
						&& ((EnumLength) shiftvalue).getEnum() == Constants.EN_SUB);
			}
		}
	}
