 ...
 */
/**
 * @WisedocChartPieShowLabelAction.java
 * 汇智互联版权所有，未经许可，不得使用
 */
...
/**
 * 类功能描述：用于设置饼状图的标签是否可见【类似于ToolTip】
 * 
 * 作者：李晓光 创建日期：2009-5-22
 */
public class WisedocChartPieShowLabelAction extends Actions {
	private final int CONSTANTS_KEY = Constants.PR_PIECHARTLENGENDLABEL_VISABLE;
	...
	public void doAction(final ActionEvent e) {
		if (e.getSource() instanceof JCommandToggleButton) {
			...
			if(btn.getActionModel().isSelected()){
				prop = new EnumProperty(Constants.EN_TRUE, "");
			}else{
				prop = new EnumProperty(Constants.EN_FALSE, "");
			}
	...
	protected void uiStateChange(final UIStateChangeEvent evt) {
		...
		if (hasPropertyKey(CONSTANTS_KEY)) {
			if (uiComponent instanceof JCommandToggleButton) {
				...
				int flag = Constants.EN_FALSE;
				if (evt.getNewValue() instanceof EnumProperty) {
					value = ((EnumProperty)evt.getNewValue());
					flag = value.getEnum();
				}
				ui.getActionModel().setSelected(flag == Constants.EN_TRUE);
				if(ui.getActionModel().isSelected()){
					ui.setIcon(MediaResource.getResizableIcon(PieChartBasicBand.SHOW));
				} else {
					ui.setIcon(MediaResource.getResizableIcon(PieChartBasicBand.HIDDEN));
				}
			}
		}
	}
