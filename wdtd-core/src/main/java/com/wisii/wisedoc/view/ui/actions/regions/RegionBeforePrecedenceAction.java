...
public class RegionBeforePrecedenceAction extends
		DefaultSimplePageMasterActions
{
	...
	private void getRibbonComponents()
	{
		...
		Set<List<Object>> comSet = RibbonUIManager.getUIComponents().get(
				RegionBefore.PRECEDENCE_ACTION).get(
	...
	public void doAction(ActionEvent e)
	{
		...
		if (e.getSource() instanceof JCheckBox)
		{
			...
			{
				current = setRegionBeforePrecedence(Constants.EN_TRUE);
			} else
			{
				current = setRegionBeforePrecedence(Constants.EN_FALSE);
			}
				...
				|| hasPropertyKey(Constants.PR_CURRENT_SIMPLE_PAGE_MASTER))
		{
			...
			if (value instanceof SimplePageMaster)
			{
				...
				if (checkBox != null)
				{
					...
					{
						int temp = ((com.wisii.wisedoc.document.attribute.RegionBefore) simValue
		...
		if (actionCommand == ActionCommandType.RIBBON_ACTION)
		{
			...
			if (RibbonUIModel.getCurrentPropertiesByType().get(this.actionType) != null)
			{
				...
				if (value != null)
				{
					...
					if (checkBox != null)
					{
						if (UiUtil.hasRegionBefore(value))
						{
							int temp = ((com.wisii.wisedoc.document.attribute.RegionBefore) value
									.getRegion(Constants.FO_REGION_BEFORE))
									.getPrecedence();
							if (temp == Constants.EN_TRUE)
							{
