...
import com.wisii.wisedoc.view.ui.ribbon.chart.ChartYAxisgrAduateBand;
 ...
 */
public class WisedocCharttOrientation extends Actions {
	private final int CONSTANTS_KEY = Constants.PR_CHART_ORIENTATION;
	...
	public void doAction(final ActionEvent e) {
		if (e.getSource() instanceof JCommandToggleButton) {
			...
			if(btn.getActionModel().isSelected()){
				prop = new EnumProperty(Constants.EN_HORIZONTAL, "");
			}else{
				prop = new EnumProperty(Constants.EN_VERTICAL, "");
			}
	...
	protected void uiStateChange(final UIStateChangeEvent evt) {
		...
		if (hasPropertyKey(CONSTANTS_KEY)) {
			if (uiComponent instanceof JCommandToggleButton) {
				...
				int flag = Constants.EN_VERTICAL;
				if (evt.getNewValue() instanceof EnumProperty) {
					value = ((EnumProperty)evt.getNewValue());
					flag = value.getEnum();
				}
				ui.getActionModel().setSelected(flag == Constants.EN_HORIZONTAL);
				if(ui.getActionModel().isSelected()){
					ui.setIcon(MediaResource.getResizableIcon(ChartYAxisgrAduateBand.AXISX));
				} else {
					ui.setIcon(MediaResource.getResizableIcon(ChartYAxisgrAduateBand.AXISY));
				}
			}
		}
	}
