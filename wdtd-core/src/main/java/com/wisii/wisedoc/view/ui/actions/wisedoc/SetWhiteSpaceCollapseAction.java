...
public class SetWhiteSpaceCollapseAction extends Actions
{
	...
	protected void uiStateChange(UIStateChangeEvent evt)
	{
		...
		if (hasPropertyKey(Constants.PR_WHITE_SPACE_COLLAPSE))
		{
	...
	private void setProperty(int value)
	{
		...
		int result = Constants.EN_TRUE;
		if (value == 1)
		{
			result = Constants.EN_FALSE;
		}
		((WiseDocDocument)getCurrentDocument()).setAttribute(Constants.PR_WHITE_SPACE_COLLAPSE,
				result,true);
	}
	...
	{
		init((Integer) getCurrentDocument().getAttribute(Constants.PR_WHITE_SPACE_COLLAPSE));
