...
public class ParagraphPositionPanel extends JPanel
{
	...
	WiseSpinner linespaceNumber = new WiseSpinner();
	{
		...
		SpinnerNumberModel number = new SpinnerNumberModel(1, 0.1, 100.000, 0.25);
	...
	private void initialize()
	{
		...
		lineSpace.addItemListener(new ItemListener()
		{
			...
			public void itemStateChanged(ItemEvent event)
			{
				if (event.getStateChange() == ItemEvent.SELECTED)
				{
					...
					int[] currentArray =
					{ Constants.PR_LINE_HEIGHT, Constants.PR_FONT_SIZE,
		...
		linespaceFixedlength.addChangeListener(new ChangeListener()
		{
			...
			public void stateChanged(ChangeEvent e)
			{
				lineHeightStrategy.setSelectedItem(lineheight);
				setAttribute(Constants.PR_LINE_HEIGHT,
						getSpaceProper((FixedLength) ((FixedLengthSpinner) e
								.getSource()).getValue()));
		...
		// FixedLength yuanshi = (FixedLength)
		// getAttributeValue(Constants.PR_FONT_SIZE);
		// FixedLength newLength = new FixedLength(new Double(yuanshi
		// .getInnerLengthValue() * 1.2).intValue(), yuanshi.getUnits());
		...
		oneLineHeight.addChangeListener(new ChangeListener()
		{
			...
			{
				lineHeightStrategy.setSelectedItem(lineheight);
				FixedLength chushi = (FixedLength) ((FixedLengthSpinner) e
						.getSource()).getValue();
				// FixedLength shezhiLength = new FixedLength(new Double(chushi
				// .getInnerLengthValue() / 1.2).intValue(), chushi
				// .getUnits());
				// setAttribute(Constants.PR_FONT_SIZE, shezhiLength);
				String current = lineSpace.getSelectedItem().toString();
				// System.out.println("xiang ::"+current);
				...
				{
					// System.out.println("she le ma ?");
					setAttribute(Constants.PR_LINE_HEIGHT,
							getSpaceProperty(chushi));
					//System.out.println("chang:"+chushi.getLengthValueString())
					// ;
					int[] currentArray =
	...
	public SpaceProperty getSpaceProperty(FixedLengthSpinner length, int key)
	{
		...
		Object object = ParagraphPropertyModel
	...
	{
		SpaceProperty old = (SpaceProperty) ParagraphPropertyModel
				.getCurrentProperty(Constants.PR_LINE_HEIGHT);
	...
	public SpaceProperty getSpaceProperty(double lengthint, int key)
	{
		SpaceProperty old = (SpaceProperty) ParagraphPropertyModel
				.getCurrentProperty(key);
		FixedLength length = (FixedLength) ParagraphPropertyModel
	...
	{
		Object oldvalue = ParagraphPropertyModel
				.getCurrentProperty(Constants.PR_LINE_HEIGHT);
		if (oldvalue != null)
		{
	...
	public String getAttribute(int key)
	{
		...
		if (map != null && map.containsKey(key))
		{
			...
			} else if (map.get(key) instanceof SpaceProperty)
			{
				SpaceProperty sp = (SpaceProperty) map.get(key);
				if (sp != null)
				{
	...
	public boolean getAttributeBoolean(int key)
	{
		...
		if (map != null && map.containsKey(key))
		{
			if (map.get(key) instanceof SpaceProperty)
			{
				SpaceProperty sp = (SpaceProperty) map.get(key);
				if (sp != null
		...
		if (map != null && map.containsKey(key))
		{
			...
			if (value instanceof SpaceProperty)
			{
				...
				} else if (length instanceof PercentLength)
				{
					...
					return (FixedLength) ((LengthBase) pelength.getBaseLength())
							.getBaseLength();
				}
			}
			return value;
	...
	{
		SpaceProperty space = (SpaceProperty) ParagraphPropertyModel
				.getCurrentProperty(key);
		if (space != null)
		{
	...
	{
		Map<Integer, Object> currentmap = new HashMap<Integer, Object>();

		for (int current : key)
		{
			currentmap.put(current, ParagraphPropertyModel
					.getCurrentProperty(current));
		}
		previewPanel.initValues(currentmap, ActionType.BLOCK, false);
	}
}
