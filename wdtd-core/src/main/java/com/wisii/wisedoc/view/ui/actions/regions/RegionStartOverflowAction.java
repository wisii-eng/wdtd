...
public class RegionStartOverflowAction extends DefaultSimplePageMasterActions
{
	...
	private void setProperty(int index)
	{
		...
		{
			case 0:
				current = setRegionStartOverflow(Constants.EN_VISIBLE);
				break;
			case 1:
				current = setRegionStartOverflow(Constants.EN_HIDDEN);
				break;
		}
	...
	public void setDefaultState(ActionCommandType actionCommand)
	{
		...
		if (actionCommand == ActionCommandType.RIBBON_ACTION)
		{
			...
			if (value != null)
			{
				int trValue = value.getRegion(Constants.FO_REGION_START)
						.getOverflow();
				getComponents();
