...
public class KnuthBlockBox extends KnuthBox {
    ...
    public KnuthBlockBox clone()
    {
		KnuthBlockBox kb = (KnuthBlockBox) super.clone();
		if (kb != null)
		{
			if (ipdRange != null)
			{
				kb.ipdRange = ipdRange.clone();
			}
			if (elementLists != null && !elementLists.isEmpty())
			{
				...
				{
					ListElement le = (ListElement) elementLists.get(i);
					els.add(le.clone());
				}
				kb.elementLists = els;
			}
			...
			{
				LinkedList els = new LinkedList();
				int size = footnoteList.size();
				for (int i = 0; i < size; i++)
				{
					ListElement le = (ListElement) footnoteList.get(i);
					els.add(le.clone());
				}
				kb.footnoteList = els;
			}
