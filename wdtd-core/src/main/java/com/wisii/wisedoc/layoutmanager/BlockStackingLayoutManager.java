...
public abstract class BlockStackingLayoutManager extends AbstractLayoutManager
    ...
    protected  CommonMarginBlock cmblock;
    protected CommonBorderPaddingBackground cbpbackground;

    /**
     * @param node the fo this LM deals with
     */
    public BlockStackingLayoutManager(CellElement node) {
        super(node);
    ...
    protected void addChildToArea(Area childArea,
        ...
        if (!(childArea instanceof Block)) {
            LogUtil.error("Child not a Block in BlockStackingLM!");
        }
    ...
    protected int updateContentAreaIPDwithOverconstrainedAdjust() {
        ...
        if (ipd < 0) {
            ...
            LogUtil.debug("Adjusting end-indent based on overconstrained geometry rules for " + fobj);
            endIndent += ipd;
            ipd = 0;
            ...
            if (returnedList != null
                ...
                if (prevLM != null) {
                    ...
                    if (mustKeepTogether()
                        ...
                        // a penalty
                    	LogUtil.warn("glue-type break possibility not handled properly, yet");
    ...
    /**
     * @see com.wisii.wisedoc.layoutmanager.BlockLevelLayoutManager#negotiateBPDAdjustment(int, com.wisii.wisedoc.layoutmanager.KnuthElement)
     */
...
/*LF*/      //log.debug("  BLM.negotiateBPDAdjustment> righe: " + returnValue);
        	...
        	LogUtil.error("BlockLayoutManager.negotiateBPDAdjustment(): unexpected Position");
            return 0;
        }
    }
...
/*LF*/  //log.debug("  BLM.getChangedKnuthElements> inizio: oldList.size() = "
        ...
        if (fobj instanceof com.wisii.wisedoc.document.Block) {
            spaceBeforeIsConditional = cmblock.getSpaceBefore().getSpace().isDiscard();
        }
        if (bpUnit > 0
            || adjustedSpaceBefore != 0) {
...
/* estensione */
        ...
        if (fobj instanceof com.wisii.wisedoc.document.Block) {
            spaceAfterIsConditional = cmblock.getSpaceAfter().getSpace().isDiscard();
        }
    ...
    private SpaceProperty getSpaceBeforeProperty() {
        if (fobj instanceof com.wisii.wisedoc.document.Block) {
            return cmblock.getSpaceBefore();
        } 
        /*else if (fobj instanceof com.wisii.wisedoc.document.BlockContainer) {
            return ((com.wisii.wisedoc.document.BlockContainer)fobj)
                .getCommonMarginBlock().spaceBefore;
        } else if (fobj instanceof com.wisii.wisedoc.document.ListBlock) {
            return ((com.wisii.wisedoc.document.ListBlock)fobj)
                .getCommonMarginBlock().spaceBefore;
        } else if (fobj instanceof com.wisii.wisedoc.document.ListItem) {
            return ((com.wisii.wisedoc.document.ListItem)fobj)
                .getCommonMarginBlock().spaceBefore;
        }*/ 
        else if (fobj instanceof com.wisii.wisedoc.document.Table) {
            return cmblock.getSpaceBefore();
        } else {
            return null;
        }
    ...
    protected boolean addKnuthElementsForBreakBefore(LinkedList returnList,
        ...
        int breakBefore = -1;
        if (fobj instanceof com.wisii.wisedoc.document.Block) {
            breakBefore = ((com.wisii.wisedoc.document.Block) fobj).getBreakBefore();
        } 
        /*else if (fobj instanceof com.wisii.wisedoc.document.BlockContainer) {
            breakBefore = ((com.wisii.wisedoc.document.BlockContainer) fobj).getBreakBefore();
        } else if (fobj instanceof com.wisii.wisedoc.document.ListBlock) {
            breakBefore = ((com.wisii.wisedoc.document.ListBlock) fobj).getBreakBefore();
        } else if (fobj instanceof com.wisii.wisedoc.document.ListItem) {
            breakBefore = ((com.wisii.wisedoc.document.ListItem) fobj).getBreakBefore();
        }*/ 
        else if (fobj instanceof com.wisii.wisedoc.document.Table) {
            breakBefore = ((com.wisii.wisedoc.document.Table) fobj).getBreakBefore();
        }
    ...
    protected boolean addKnuthElementsForBreakAfter(LinkedList returnList,
        ...
        int breakAfter = -1;
        if (fobj instanceof com.wisii.wisedoc.document.Block) {
            breakAfter = ((com.wisii.wisedoc.document.Block) fobj).getBreakAfter();
        } 
        /*else if (fobj instanceof com.wisii.wisedoc.document.BlockContainer) {
            breakAfter = ((com.wisii.wisedoc.document.BlockContainer) fobj).getBreakAfter();
        } else if (fobj instanceof com.wisii.wisedoc.document.ListBlock) {
            breakAfter = ((com.wisii.wisedoc.document.ListBlock) fobj).getBreakAfter();
        } else if (fobj instanceof com.wisii.wisedoc.document.ListItem) {
            breakAfter = ((com.wisii.wisedoc.document.ListItem) fobj).getBreakAfter();
        }*/ 
        else if (fobj instanceof com.wisii.wisedoc.document.Table) {
            breakAfter = ((com.wisii.wisedoc.document.Table) fobj).getBreakAfter();
        }
...
//                        (!spaceAfter.getSpace().isDiscard()) ? false : true));
    ...
    protected LinkedList createUnitElements(LinkedList oldList) {
        ...
        if (fobj instanceof com.wisii.wisedoc.document.Block) {
            correctFirstElement = cmblock.getSpaceBefore().getSpace().isDiscard();
        }
        ...
        if (fobj instanceof com.wisii.wisedoc.document.Block) {
            correctLastElement =cmblock.getSpaceAfter().getSpace().isDiscard();
        }
