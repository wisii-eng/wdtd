 ...
 */
/**
 * @SVGContainerContainerLayoutManager.java
 * 汇智互联版权所有，未经许可，不得使用
 */
 ...
 * 类功能描述：SVG容器的排版类
 * 
 * 作者：zhangqiang 创建日期：2009-2-27
 */
public class SVGContainerLayoutManager extends BlockStackingLayoutManager
		implements ConditionalElementListener
{
	 ...
	 */
	public SVGContainerLayoutManager(SVGContainer node)
	{
		super(node);
	}
			...
			int alignment)
	{
		...
		contentRectOffsetX += cmblock.getStartIndent().getValue(this);
	...
	private class BlockContainerBreaker extends AbstractBreaker
	{
		...
		private SVGContainerLayoutManager bclm;
		...
		public BlockContainerBreaker(SVGContainerLayoutManager bclm,
				MinOptMax ipd)
		{
			this.bclm = bclm;
			this.ipd = ipd;
			...
			LayoutContext layoutContext)
	{
		...
		getPSLM().notifyEndOfLayout(((SVGContainer) getFObj()).getId());
	}
	...
	public Area getParentArea(Area childArea)
	{
		if (referenceArea == null)
		{
			...
			TraitSetter.setProducerID(viewportBlockArea, getBlockContainerFO()
					.getId());
			TraitSetter.addBorders(viewportBlockArea, getBlockContainerFO()
					.getCommonBorderPaddingBackground(), discardBorderBefore,
					discardBorderAfter, false, false, this);
			TraitSetter.addPadding(viewportBlockArea, getBlockContainerFO()
			...
			// this);
			TraitSetter.addMargins(viewportBlockArea, getBlockContainerFO()
					.getCommonBorderPaddingBackground(), startIndent,
					endIndent, this);
	...
	protected void flush()
	{
		...
		/* 【添加：END】by 李晓光 2008-10-31 */
		TraitSetter.addBackground(viewportBlockArea, getBlockContainerFO()
	 ...
	 */
	protected SVGContainer getBlockContainerFO()
	{
		return (SVGContainer) fobj;
	}
