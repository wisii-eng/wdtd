...
import com.wisii.wisedoc.layoutmanager.table.TableContentLayoutManager.GridUnitPart;
import com.wisii.wisedoc.layoutmanager.table.TableContentLayoutManager.TableContentPosition;
import com.wisii.wisedoc.layoutmanager.table.TableContentLayoutManager.TableHFPenaltyPosition;
...
public class TableStepper {
    ...
    private void calcTotalHeight() {
        ...
        LogUtil.debug("totalHeight=" + totalHeight);
    }
    ...
    private int getMaxRemainingHeight() {
        ...
        //LogUtil.debug("maxRemainingHeight=" + maxW);
        return maxW;
    }
    ...
    public LinkedList getCombinedKnuthElementsForRowGroup(
        ...
        while ((step = getNextStep(laststep)) >= 0) {
            ...
            //LogUtil.debug(">>> guPARTS: " + gridUnitParts);
            ...
            lastTCPos = tcpos;
        	LogUtil.debug(" - backtrack=" + rowBacktrackForLastStep
                        + " - row=" + activeRow + " - " + tcpos);
            ...
            if (isBreakCondition()) {
                if (skippedStep) {
                	LogUtil.error("This is a conflict situation. The output may be wrong."
            ...
            LogUtil.debug("step=" + step + " (+" + increase + ")"
                    + " box=" + boxLen
                    + " penalty=" + penaltyLen
    ...
    private int getNextStep(int lastStep) {
        ...
        if (rowPendingIndicator == 0) {
            if (activeRow < rowGroup.length - 1) {
                ...
                if (rowFO != null && rowFO.getBreakAfter() != Constants.EN_AUTO) {
                	LogUtil.warn("break-after ignored on table-row because of row spanning " + rowFO);
                ...
                activeRow++;
            	LogUtil.debug("===> new row: " + activeRow);
                ...
                if (rowFO != null && rowFO.getBreakBefore() != Constants.EN_AUTO) {
                	LogUtil.warn("break-before ignored on table-row because of row spanning " + rowFO);
                	/*LogUtil.warn(FONode.decorateWithContextInfo(
        ...
        for (int i = 0; i < start.length; i++) {
            ...
            while (end[i] + 1 < elementLists[i].size()) {
                ...
                if (el.isPenalty()) {
                    if (el.getP() <= -KnuthElement.INFINITE) {
                    	LogUtil.debug("FORCED break encountered!");
                        forcedBreaks[i] = true;
                        break;
            ...
            //LogUtil.debug("part " + start[i] + "-" + end[i] + " " + widths[i]);
            if (end[i] + 1 >= elementLists[i].size()) {
        	...
        	LogUtil.debug("borders before=" + borderBefore[i] + " after=" + borderAfter[i]);
        	LogUtil.debug("padding before=" + paddingBefore[i] + " after=" + paddingAfter[i]);
        }
        if (seqCount == 0) {
            return -1;
        }
    	...
    	LogUtil.debug("candidate steps: " + sb + " lastStep=" + lastStep);
        ...
        for (int i = 0; i < widths.length; i++) {
            ...
            if (len > minStep) {
                ...
                if (baseWidth[i] + widths[i] > minStep) {
                	LogUtil.debug("minStep vs. border/padding increase conflict:");
                    if (activeRow == 0) {
                    	LogUtil.debug("  First row. Skip this step.");
                        skippedStep = true;
                    } else {
                    	LogUtil.debug("  row-span situation: backtracking to last row");
