...
import com.wisii.wisedoc.layoutmanager.ElementListUtils;
...
import com.wisii.wisedoc.layoutmanager.MinOptMaxUtil;
import com.wisii.wisedoc.layoutmanager.Position;
import com.wisii.wisedoc.layoutmanager.PositionIterator;
import com.wisii.wisedoc.layoutmanager.SpaceResolver;
import com.wisii.wisedoc.layoutmanager.TraitSetter;
import com.wisii.wisedoc.layoutmanager.SpaceResolver.SpaceHandlingBreakPosition;
...
public class TableContentLayoutManager implements PercentBaseContext {
	...
	public LinkedList getNextKnuthElements(LayoutContext context, int alignment) {
        LogUtil.debug("==> Columns: " + getTableLM().getColumns());
        ...
        if (headerIter != null && headerList == null) {
            ...
            if (LogUtil.canDebug()) {
            	LogUtil.debug("==> Header: " + headerNetHeight + " - " + this.headerList);
            }
        ...
        if (footerIter != null && footerList == null) {
        	...
        	LogUtil.debug("==> Footer: " + footerNetHeight + " - " + this.footerList);
    ...
    private LinkedList getKnuthElementsForRowIterator(TableRowIterator iter,
        ...
        while ((rowGroup = iter.getNextRowGroup()) != null) {
            ...
            if (rowFO != null && rowFO.getBreakBefore() != Constants.EN_AUTO) {
            	LogUtil.info("break-before found");
            ...
            if (context.isKeepWithNextPending()) {
            	LogUtil.debug("child LM (row group) signals pending keep-with-next");
            }
            if (context.isKeepWithPreviousPending()) {
            	LogUtil.debug("child LM (row group) signals pending keep-with-previous");
                if (returnList.size() > 0) {
                    //Modify last penalty
                    ListElement last = (ListElement)returnList.getLast();
                    if (last.isPenalty()) {
    ...
    private void createElementsForRowGroup(LayoutContext context, int alignment,
            ...
            EffRow[] rowGroup) {
    	LogUtil.debug("Handling row group with " + rowGroup.length + " rows...");
                ...
                if ((gu.isPrimary() || (gu.getColSpanIndex() == 0 && gu.isLastGridUnitRowSpan()))
                    ...
                    if (gu.isPrimary()) {
                        ...
                        if ((elems.size() > 0)
                        	...
                        	LogUtil.debug("Descendant of table-cell signals break: "
                        ...
                        if (childLC.isKeepWithNextPending()) {
                        	LogUtil.debug("child LM signals pending keep-with-next");
                            primary.setFlag(GridUnit.KEEP_WITH_NEXT_PENDING, true);
                        }
                        if (childLC.isKeepWithPreviousPending()) {
                        	LogUtil.debug("child LM signals pending keep-with-previous");
        ...
        if (LogUtil.canDebug()) {
        	LogUtil.debug("rowGroup:");
            for (int i = 0; i < rowHeights.length; i++) {
            	LogUtil.debug("  height=" + rowHeights[i] + " explicit=" + explicitRowHeights[i]);
            }
        }
    ...
    public void addAreas(PositionIterator parentIter, LayoutContext layoutContext) {
        ...
        if (lastPos instanceof TableHFPenaltyPosition) {
            ...
            LogUtil.debug("Break at penalty!");
        ...
        /* 【删除】 by 李晓光  该版本还不支持Marker 2008-09-02*/
       /* Map markers = getTableLM().getTable().getMarkers();
        ...
        /* 【删除】 by 李晓光 还不支持Marker 2008-09-02  */
        /*if (markers != null) {
            getTableLM().getCurrentPV().addMarkers(markers,
        ...
        while (iterator.hasNext()) {
            ...
            if (pos instanceof TableContentPosition) {
                ...
                if (tcpos.getFlag(TableContentPosition.LAST_IN_ROWGROUP)
                    ...
                    lastPos = true;
                    /* 【删除】 by 李晓光  目前还不支持Marder  2008-09-02 */
                ...
                if (LogUtil.canDebug()) {
                	LogUtil.debug("Ignoring position: " + pos);
                }
            }
        }
        if (body != null) {
    ...
    private class RowPainter {
        ...
        public void handleTableContentPosition(TableContentPosition tcpos) {
            ...
            if (LogUtil.canDebug()) {
            	LogUtil.debug("===handleTableContentPosition(" + tcpos);
            }
            ...
            while (partIter.hasNext()) {
                ...
                if (gridUnits[colIndex] != gup.pgu) {
                    if (gridUnits[colIndex] != null) {
                    	LogUtil.warn("Replacing GU in slot " + colIndex
        ...
        public int addAreasAndFlushRow(boolean forcedFlush) {
            ...
            if (LogUtil.canDebug()) {
            	LogUtil.debug("Remembering yoffset for row " + lastRow.getIndex() + ": " + yoffset);
            }
            ...
            for (int i = 0; i < gridUnits.length; i++) {
                if ((gridUnits[i] != null)
                    ...
                    if (LogUtil.canDebug()) {
                    	LogUtil.debug("getting len for " + i + " "
                                + start[i] + "-" + end[i]);
                    }
                    readyCount++;
            ...
            for (int i = 0; i < gridUnits.length; i++) {
                ...
                if ((gridUnits[i] != null)
                    ...
                    if (LogUtil.canDebug()) {
                    	LogUtil.debug((forcedFlush ? "FORCED " : "") + "flushing..." + i + " "
                                + start[i] + "-" + end[i]);
                    }
        ...
        private void addAreasForCell(PrimaryGridUnit pgu, int startPos, int endPos,
            ...
            if (LogUtil.canDebug()) {
            	LogUtil.debug("Creating area for cell:");
            	LogUtil.debug("  current row: " + row.getIndex());
            	LogUtil.debug("  start row: " + pgu.getStartRow() + " " + yoffset + " " + effYOffset);
            	LogUtil.debug("  contentHeight: " + contentHeight + " rowHeight=" + rowHeight
    ...
    public Block getRowArea(TableRow row) {
        if (row == null || !row.getCommonBorderPaddingBackground().hasBackground()) {
            ...
            block.setAreaKind(AreaKind.BACKGROUND);
