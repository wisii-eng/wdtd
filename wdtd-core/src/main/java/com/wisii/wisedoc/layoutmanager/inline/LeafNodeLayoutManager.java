...
import com.wisii.wisedoc.layoutmanager.AbstractLayoutManager;
...
public abstract class LeafNodeLayoutManager extends AbstractLayoutManager
    ...
    /**
     * 获得其父LineLayoutManager
     * @param layout						指定LayoutManager。
     * @return	{@link LineLayoutManager}	返回当前LayoutManager的父LineLayoutManager，如果没有返回Null。
     */
    private LineLayoutManager getLineManager(LayoutManager layout){
    	if(layout == null)
    		return null;
    	if(layout instanceof LineLayoutManager)
    		return (LineLayoutManager)layout;
    	
    	return getLineManager(layout.getParent());
    }
    /**
     * 更新当前InlineArea中的起始索引，和元素数目。
     * @param inline		指定要更新的Inline-area。
     */
    protected void updateCurrentOffset(InlineArea inline){
    	 LineLayoutManager line = getLineManager(this);
         if(inline instanceof TextArea){
        	 ...
        	 if(needUp(inline)){
        		 text.setStartIndex(line.getCurrentOffset());
        		 line.addOffset(elementCount);
        	 }
        	 text.setElementCount(elementCount);
         }else if(inline instanceof Viewport){
        	 Viewport view = (Viewport)inline;
        	 view.setStartIndex(line.getCurrentOffset());
        	 elementCount ++;
    ...
    /**
     * 判断是否要更新当前inline-area的起始索引，元素数目。
     * 1.area.isWhole() == true;
     * 2.发生断行。
     * 3.在不同的area中，应该具有相同的startIindex、elementCount。
     * @param inline			指定inline-area。
     * @return	{@link Boolean}	需要更新返回True，否则：False。
     */
    private boolean needUp(InlineArea inline){
    	CellElement source = inline.getSource();
    	if(!(source instanceof FOText))
    		return Boolean.TRUE;
    	FOText text = (FOText)source;
    	if(!text.isWhole())
    		return Boolean.TRUE;
    	
    	List<TextArea> areas = text.getAreas();
    	int index = areas.indexOf(inline);
    	if(index != 0){
    		TextArea t = areas.get(0);
    		TextArea t0 = (TextArea)inline;
    		t0.setStartIndex(t.getStartIndex());
    	}
    	return (index == 0);
    }
     ...
     */
    public LeafNodeLayoutManager(CellElement node) {
        super(node);
    }
    ...
    public void addAreas(PositionIterator posIter, LayoutContext context) {
    	//【添加：START】 by 李晓光  2010-1-28
//    	elementCount = 0;
    	//【添加：END】 by 李晓光  2010-1-28
        addId();
    ...
    public void removeWordSpace(List oldList) {
        // do nothing
    	LogUtil.warn(this.getClass().getName() + " should not receive a call to removeWordSpace(list)");
    }
