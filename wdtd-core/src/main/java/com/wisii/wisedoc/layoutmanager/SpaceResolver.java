...
public class SpaceResolver {
    ...
    public SpaceResolver(List first, BreakElement breakPoss, List second,
        ...
        if (breakPoss != null) {
            if (breakPoss.getPendingAfterMarks() != null) {
                if (LogUtil.canDebug()) {
                	LogUtil.debug("    adding pending before break: "
            ...
            if (breakPoss.getPendingBeforeMarks() != null) {
                if (LogUtil.canDebug()) {
                	LogUtil.debug("    adding pending after break: "
        ...
        if (LogUtil.canDebug()) {
        	...
        	LogUtil.debug("  break: " + breakPoss);
        	LogUtil.debug("after: " + second);
        	LogUtil.debug("NO-BREAK: " + toString(noBreak, noBreakLengths));
        }
        ...
        for (int i = 0; i < elems.length; i++) {
            ...
            if (elems[effIndex] instanceof BorderOrPaddingElement) {
                ...
                if (bop.isConditional() && !(bop.isFirst() || bop.isLast())) {
                    if (LogUtil.canDebug()) {
                        LogUtil.debug("Nulling conditional element: " + bop);
                    }
                    lengths[effIndex] = null;
                }
            }
        }
        if (LogUtil.canDebug() && elems.length > 0) {
            LogUtil.debug("-->Resulting list: " + toString(elems, lengths));
        }
    }
    ...
    private void performSpaceResolutionRule1(UnresolvedListElement[] elems, MinOptMax[] lengths,
        ...
        for (int i = 0; i < elems.length; i++) {
            ...
            if (LogUtil.canDebug()) {
                LogUtil.debug("Nulling conditional element using 4.3.1, rule 1: " + elems[effIndex]);
            }
            lengths[effIndex] = null;
        }
        if (LogUtil.canDebug() && elems.length > 0) {
    ...
    private void performSpaceResolutionRules2to3(UnresolvedListElement[] elems,
        ...
        if (LogUtil.canDebug()) {
            LogUtil.debug("rule 2-3: " + start + "-" + end);
        }
        SpaceElement space;
        int remaining;
        ...
        if (hasForcing) {
            for (int i = start; i <= end; i++) {
                ...
                if (!space.isForcing()) {
                    if (LogUtil.canDebug()) {
                        LogUtil.debug("Nulling non-forcing space-specifier using 4.3.1, rule 2: "
                                + elems[i]);
                    }
                    lengths[i] = null;
                }
        ...
        if (highestPrecedence != 0 && LogUtil.canDebug()) {
            LogUtil.debug("Highest precedence is " + highestPrecedence);
        }
        ...
        for (int i = start; i <= end; i++) {
            ...
            if (space.getPrecedence() != highestPrecedence) {
                if (LogUtil.canDebug()) {
                    LogUtil.debug("Nulling space-specifier with precedence "
        ...
        if (LogUtil.canDebug()) {
            LogUtil.debug("Greatest optimum: " + greatestOptimum);
        }
        if (remaining <= 1) {
            return;
        }
        ...
        for (int i = start; i <= end; i++) {
            ...
            if (space.getLength().opt < greatestOptimum) {
                if (LogUtil.canDebug()) {
                    LogUtil.debug("Nulling space-specifier with smaller optimum length "
                            + "using 4.3.1, rule 3: "
                            + elems[i]);
                }
                lengths[i] = null;
        ...
        for (int i = start; i <= end; i++) {
            ...
            if (remaining > 1) {
                if (LogUtil.canDebug()) {
                    LogUtil.debug("Nulling non-last space-specifier using 4.3.1, rule 3, second part: "
                            + elems[i]);
                }
                lengths[i] = null;
                remaining--;
        ...
        if (LogUtil.canDebug() && elems.length > 0) {
            LogUtil.debug("Remaining spaces: " + remaining);
            LogUtil.debug("-->Resulting list: " + toString(elems, lengths));
        }
    }
    ...
    private void resolve() {
        if (breakPoss != null) {
            ...
            if (hasFirstPart()) {
                ...
                LogUtil.debug("Swapping first and second parts.");
    ...
    private void generate(ListIterator iter) {
        ...
        if (LogUtil.canDebug()) {
            LogUtil.debug("noBreakLength=" + noBreakLength
                    + ", glue1=" + glue1
    ...
    public static void resolveElementList(LinkedList elems) {
        ...
        while (iter.hasNext()) {
            ...
            if (el.isUnresolvedElement()) {
                if (LogUtil.canDebug()) {
                    LogUtil.debug("unresolved found: " + el + " " + first + "/" + last);
                }
                ...
                if (breakPoss == null && unresolvedSecond.size() == 0 && !last) {
                    LogUtil.debug("Swap first and second parts in no-break condition,"
                ...
                LogUtil.debug("----start space resolution (first=" + first + ", last=" + last + ")...");
                ...
                LogUtil.debug("----end space resolution.");
            }
            first = false;
        }
    }
