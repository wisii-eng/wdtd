 ...
 * 创建日期：2009-9-3
 */
public class ConnwithAttWriter extends AbstractAttributeWriter
{
	...
	{
		if (value == null || !(value instanceof ConnWith))
		{
			return "";
		}
		String returns = "";
		ConnWith connwith = (ConnWith) value;
		returns = returns + getKeyName(key);
		returns = returns + EQUALTAG + QUOTATIONTAG
				+ DocumentWirter.getConnWithID(connwith) + QUOTATIONTAG
				+ SPACETAG;
		return returns;
	}
