 ...
 */
public class EnumNumberReader extends EnumPropertyReader {
	/*
	 * (non-Javadoc)
	 * 
	...
	public Object read(int key, String value)
	{
		...
		{
			int enumvalue = getEnumKey(value);
			if (enumvalue > -1)
			{
				return new EnumNumber(enumvalue, -1);
			} else
			{
				LogUtil.debug("EnumNumber类型的属性字符串不可解析，属性字符串为：" + value);
			}
		}
		return null;
	}
