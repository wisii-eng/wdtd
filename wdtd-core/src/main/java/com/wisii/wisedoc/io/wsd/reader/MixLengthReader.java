...
/**
 * 类功能描述：多种类型的length读写类；
 *
 * 作者：zhangqiang
 * 创建日期：2008-12-19
 */
public class MixLengthReader extends SingleAttributeReader
{

	private FixedLengthReader fixlenreader = new FixedLengthReader();
	private EnumLengthReader enumlenreader = new EnumLengthReader();
	private PercentLengthReader percentlenreader = new PercentLengthReader();
	/* (non-Javadoc)
	...
	{
		Object length = percentlenreader.read(key, value);
		if (length != null)
		{
			return length;
		}
		 length = fixlenreader.read(key, value);
		if (length != null)
		{
			return length;
		}
		length = enumlenreader.read(key, value);
		if (length != null)
		{
			return length;
		}
