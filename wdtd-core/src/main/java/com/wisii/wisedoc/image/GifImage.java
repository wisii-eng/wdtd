...
public class GifImage extends AbstractFovImage {
    ...
    protected boolean loadBitmap() {
        ...
        try {
            ...
            try {
            	...
            	LogUtil.errorException("Image grabbing interrupted : "
                              + ex.getMessage(), ex);
                return false;
            }
        	...
        	LogUtil.errorException("Error while loading image (Gif): " + ex.getMessage(), ex);
            return false;
        } finally {
	...
	public GifImage clone()
	{
		GifImage image = new GifImage(this.imageInfo);
		return image;
	}
}
