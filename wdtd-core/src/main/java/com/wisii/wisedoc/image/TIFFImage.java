...
public class TIFFImage extends XmlGraphicsCommonsImage {
    ...
    /**
     * @see com.wisii.wisedoc.image.XmlGraphicsCommonsImage#decodeImage(
	...
	{
		org.apache.xmlgraphics.image.codec.tiff.TIFFImage img = new org.apache.xmlgraphics.image.codec.tiff.TIFFImage(
				stream, null, 0);
		...
		if (fld != null)
		{
			...
			default:
				// ignored
				LogUtil.warn("Cannot determine bitmap resolution."
		...
		{
			stripCount = (int) (dir
					.getFieldAsLong(TIFFImageDecoder.TIFF_IMAGE_LENGTH) / fld
					.getAsLong(0));
		}
		...
		stripOffset = dir.getField(TIFFImageDecoder.TIFF_STRIP_OFFSETS)
				.getAsLong(0);
		stripLength = dir.getField(TIFFImageDecoder.TIFF_STRIP_BYTE_COUNTS)
				.getAsLong(0);
     ...
     * @see com.wisii.wisedoc.image.AbstractFovImage#loadOriginalData()
     */
        ...
        if (loadDimensions()) {
            ...
            try {
                ...
                if (bytesRead != stripLength) {
                	LogUtil.error("Error while loading image: length mismatch on read");
                    return false;
                }
                ...
                LogUtil.errorException("Error while loading image strip 1 (TIFF): ", ioe);
                return false;
            } finally {
	...
	{
		TIFFImage image = new TIFFImage(this.imageInfo);
		return image;
	}
}
