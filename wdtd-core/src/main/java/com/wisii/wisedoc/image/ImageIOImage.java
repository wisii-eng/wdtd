...
import com.wisii.wisedoc.util.UnitConv;
...
public class ImageIOImage extends AbstractFovImage {
    ...
    protected boolean loadBitmap() {
        ...
        try {
            ...
            if (!iter.hasNext()) {
                LogUtil.error("No ImageReader found.");
                return false;
            }
            ...
            LogUtil.errorException("Error while loading image: " + ex.getMessage(), ex);
            return false;
        } finally {
	...
	public ImageIOImage clone()
	{
		ImageIOImage image = new ImageIOImage(this.imageInfo);
		return image;
	}
}
