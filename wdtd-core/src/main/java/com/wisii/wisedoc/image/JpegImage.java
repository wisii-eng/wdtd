...
import com.wisii.wisedoc.util.CMYKColorSpace;
...
public class JpegImage extends AbstractFovImage {
    ...
    protected boolean loadOriginalData() {
        ...
        try {
        	...
        	LogUtil.errorException("Error while loading image (Jpeg): " + ex.getMessage(), ex);
            return false;
        } finally {
        ...
        if (this.raw.length > (index + 2)
                    ...
                    if (uByte(this.raw[index + 1]) == 192 /*0xC0*/
                        ...
                        if (this.raw[index + 9] == 1) {
                            ...
                            */
                            ...
                            LogUtil.error("Unknown ColorSpace for image: "
                                                   + "");
                            return false;
                        }
        	...
        	LogUtil.error("Error while loading "
        ...
        if (iccStream != null && iccStream.size() > 0) {
            ...
            if (padding != 0) {
                try {
                	...
                	LogUtil.errorException("Error while aligning ICC stream: " + ex.getMessage(), ex);
                    return false;
                }
            }
            try {
            	...
            	LogUtil.errorException("Invalid ICC profile: " + e, e);
                return false;
            }
        } else if (this.colorSpace == null) {
        	LogUtil.error("ColorSpace not specified for JPEG image");
            return false;
        }
	...
	{
		JpegImage image = new JpegImage(this.imageInfo);
		return image;
	}
}
