    ...
    private FovImage.ImageInfo loadImage(String uri, InputStream bis,
        ...
        if (batik) {
            try {
                ...
                batik = false;
                LogUtil.warnException("Batik not in class path", e);
                return null;
            }
        }
        return null;
    ...
    class Loader {
        private FovImage.ImageInfo getImage(String uri, InputStream fis,
            ...
            try {
                ...
                batik = false;
                LogUtil.warnException("Batik not in class path", ncdfe);
                return null;
            } catch (Exception e) {
                // If the svg is invalid then it throws an IOException
                // so there is no way of knowing if it is an svg document

            	LogUtil.debug("Error while trying to load stream as an SVG file: "
                                       + e.getMessage());
