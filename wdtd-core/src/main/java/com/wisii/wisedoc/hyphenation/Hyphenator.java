...
public class Hyphenator {
    ...
    public static HyphenationTree getHyphenationTree(String lang,
        ...
        if (hTree != null) {
        	...
        	LogUtil.error("Couldn't find hyphenation pattern " + key);
    ...
    private static HyphenationTree readHyphenationTree(InputStream in) {
        ...
        try {
        	...
        	LogUtil.errorException("I/O error while loading precompiled hyphenation pattern file", ioe);
        } catch (ClassNotFoundException cnfe) {
        	LogUtil.errorException("Error while reading hyphenation object from file", cnfe);
        }
        return hTree;
    }
        ...
        try {
            ...
            if (is == null) {
                if (key.length() == 5) {
                    ...
                    if (is != null) {
                        if (LogUtil.canDebug()) {
                        	LogUtil.debug("Couldn't find hyphenation pattern '"
                                    + key
                        ...
                        if (LogUtil.canDebug()) {
                        	LogUtil.debug("Couldn't find precompiled hyphenation pattern "
                                    + lang + " in resources.");
                        }
                        return null;
                    }
    ...
    public static HyphenationTree getUserHyphenationTree(String key,
        ...
        if (source != null) {
            try {
                ...
                if (LogUtil.canDebug()) {
                	LogUtil.debugException("I/O problem while trying to load " + name, ioe);
                }
            }
        }
        ...
        if (source != null) {
            ...
            try {
            	...
            	LogUtil.error("Can't load user patterns from XML file " + source.getSystemId()
                        + ": " + ex.getMessage());
                return null;
            } catch (IOException ioe) {
                if (LogUtil.canDebug()) {
            ...
            if (LogUtil.canDebug()) {
            	LogUtil.debug("Could not load user hyphenation file for '" + key + "'.");
            }
            return null;
        }
    }
