...
public class RegionAfter extends RegionBA
{
	...
	/**
	 * 初始化过程的描述，初始化页脚需要9个参数，按顺序分别是 解释 | 默认值 1、页脚的优先级 | Constants.EN_FALSE 或者
	 * EnumNumber precedence = new EnumNumber(-1,0); 根据规范precedence的默认值为false
	 * 2、页脚的高度 | 一般为FixedLength类型的属性，比如：Length extent = new FixedLength(1d,
	 * "cm"); 3、页脚的页布局 | null 4、页脚的背景和边框属性 |
	 * null（根据FO规范region-body的border和padding必须为0） 5、页脚的基线位置 | Constants.EN_TOP
	 * 6、溢出处理 | Constants.EN_AUTO 7、页脚区域的名称 | null 8、页脚的referenceOrientation | 0
	 * 9、页脚的写作模式 | Constants.EN_LR_TB
	 * 
	 * @param 初始化参数说明
	 * @exception 说明在某情况下
	 *                ,将发生什么异常}
	 */
	public RegionAfter(int precedence, Length extent,
	...
	{
		return Constants.FO_REGION_AFTER;
	}

	public RegionAfter clone()
	{
		...
		RegionAfter region = new RegionAfter(precedence, extent, null,
