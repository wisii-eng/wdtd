...
public class FOURIResolver implements javax.xml.transform.URIResolver
{
    ...
    public Source resolve(String href, String base) throws javax.xml.transform.TransformerException
    {
        ...
        if (f.exists())
        {
            ...
            {
                LogUtil.errorException("Could not convert filename to URL: " + mfue.getMessage(), mfue);
                return null;
            }
        }
        else
        {
            ...
            if(baseURL == null)
            {
                ...
                catch(MalformedURLException mue)
                {
                    ...
                    {
                    	LogUtil.errorException("Error with URL '" + href + "': " + mue.getMessage(), mue);
                        return null;
                    }
                }
            }
            else
            {
                ...
                {
                	LogUtil.errorException("Error with URL '" + href + "': " + mfue.getMessage(), mfue);
                    return null;
                }
            }
        } //else end.
        ...
        catch (FileNotFoundException fnfe)
        {
        	...
        	LogUtil.debug("File not found: " + effURL);
        }
        catch (java.io.IOException ioe)
        {
        	LogUtil.errorException("Error with opening URL '" + href + "': " + ioe.getMessage(), ioe);
        }
        return null;
    }
...
//            return new URL(baseURL == null ? new java.io.File("").toURL().toExternalForm() : baseURL);
        ...
        else
        {
            ...
            catch(MalformedURLException mue)
            {
                ...
                {
                	LogUtil.errorException("Error with URL '" + baseURL + "': " + mue.getMessage(), mue);
                    return null;
                }
            }
        }
