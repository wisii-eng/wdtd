...
public class Page extends AreaTreeObject implements Serializable, Cloneable
{
	...
	private PageViewport parent = null;
	...
	public PageViewport getParent() {
		return parent;
	}

	public void setParent(PageViewport parent) {
		this.parent = parent;
	}
	...
	public Page(SimplePageMaster spm) {
		...
		Length marginTop = mProps.getMarginTop();
		Length marginLeft = mProps.getMarginLeft();
		Length marginRight = mProps.getMarginRight();
		Length marginBottom = mProps.getMarginBottom();
		Rectangle pageRefRect
			=  new Rectangle(marginLeft.getValue(pageWidthContext)
							, marginTop.getValue(pageHeightContext)
							, pageViewPortDims.ipd
								- marginLeft.getValue(pageWidthContext)
								- marginRight.getValue(pageWidthContext)
							, pageViewPortDims.bpd
								- marginTop.getValue(pageHeightContext)
								- marginBottom.getValue(pageHeightContext));
		
		/*Rectangle pageRefRect =  new Rectangle(0, 0, pageViewPortDims.ipd, pageViewPortDims.bpd);*/
		...
		for (Iterator regenum = spm.getRegions().values().iterator();
			...
			setRegionReferencePosition(rr, r, rvp.getViewport());
