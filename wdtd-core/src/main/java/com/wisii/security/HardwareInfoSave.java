...
public class HardwareInfoSave
{
	...
	{
		/*【20090828 刘晓修改将】
		if (filepath == null || filepath.trim().isEmpty() || diskid == null
			【替换为】	
		...
		try
		{
			...
			byte[] datas = base64encoder.encode((HardWareInfoGetter.getDiskID() +"," + HardWareInfoGetter.getMac()+"," + HardWareInfoGetter.getProcessorid()).getBytes()).getBytes();
			out.write(datas);
			return true;
